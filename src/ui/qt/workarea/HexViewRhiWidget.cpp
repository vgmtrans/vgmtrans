...
void HexViewRhiWidget::initIfNeeded() {
  if (m_inited) {
    return;
  }
...
void HexViewRhiWidget::renderFrame(QRhiCommandBuffer* cb) {
  initIfNeeded();
  ...
  if (currentPixelSize.isEmpty()) {
    if (debugLoggingEnabled()) {
      qDebug() << "HexViewRhiWidget renderFrame skipped: render target pixel size empty";
    }
    return;
  }

  if (debugLoggingEnabled() && !m_loggedFrame) {
    qDebug() << "HexViewRhiWidget frame started"
...
void HexViewRhiWidget::ensureRenderTargets(const QSize& pixelSize) {
...
void HexViewRhiWidget::releaseRenderTargets() {
  delete m_contentRp;
  m_contentRp = nullptr;
  delete m_contentRt;
  m_contentRt = nullptr;
...
bool HexViewRhiWidget::debugLoggingEnabled() const {
  return isRhiDebugEnabled();
}

void HexViewRhiWidget::ensurePipelines() {
  QRhiRenderTarget* rt = renderTarget();
...
void HexViewRhiWidget::ensureGlyphTexture(QRhiResourceUpdateBatch* u) {
...
void HexViewRhiWidget::updateUniforms(QRhiResourceUpdateBatch* u, float scrollY,
...
bool HexViewRhiWidget::ensureInstanceBuffer(QRhiBuffer*& buffer, int bytes) {
  if (bytes <= 0) {
    bytes = 1;
  }
...
void HexViewRhiWidget::updateInstanceBuffers(QRhiResourceUpdateBatch* u) {
...
void HexViewRhiWidget::drawRectBuffer(QRhiCommandBuffer* cb, QRhiBuffer* buffer, int count,
...
void HexViewRhiWidget::drawGlyphBuffer(QRhiCommandBuffer* cb, QRhiBuffer* buffer, int count,
                                      int firstInstance) {
  if (!buffer || count <= 0) {
    return;
  }
...
void HexViewRhiWidget::drawFullscreen(QRhiCommandBuffer* cb, QRhiGraphicsPipeline* pso,
...
QRectF HexViewRhiWidget::glyphUv(const QChar& ch) const {
...
void HexViewRhiWidget::appendRect(std::vector<RectInstance>& rects, float x, float y, float w, float h,
                                 const QVector4D& color) {
  rects.push_back({x, y, w, h, color.x(), color.y(), color.z(), color.w()});
}

void HexViewRhiWidget::appendGlyph(std::vector<GlyphInstance>& glyphs, float x, float y, float w, float h,
...
void HexViewRhiWidget::ensureCacheWindow(int startLine, int endLine, int totalLines) {
...
void HexViewRhiWidget::rebuildCacheWindow() {
  m_cachedLines.clear();
...
const HexViewRhiWidget::CachedLine* HexViewRhiWidget::cachedLineFor(int line) const {
...
void HexViewRhiWidget::buildBaseInstances() {
...
void HexViewRhiWidget::buildSelectionInstances(int startLine, int endLine) {
