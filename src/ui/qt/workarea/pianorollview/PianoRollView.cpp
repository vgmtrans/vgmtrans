...
PianoRollView::PianoRollView(QWidget* parent)
  ...
  connect(m_playbackAutoScrollAnimation, &QVariantAnimation::valueChanged, this, [this](const QVariant& value) {
    ...
    const int nextValue = std::clamp(static_cast<int>(std::lround(value.toDouble())),
                                     hbar->minimum(),
                                     hbar->maximum());
    if (m_rhiHost->syncPlaybackAutoScrollToRenderFrame()) {
...
void PianoRollView::setSequence(VGMSeq* seq) {
  ...
  m_noteSelectionAppendMode = false;
  m_noteSelectionBaseIndices.clear();
...
void PianoRollView::setSelectedItems(const std::vector<const VGMItem*>& items,
  ...
  const VGMItem* previousPrimary = primarySelectedItem();
  ...
  if (primarySelectedItem()) {
    for (size_t index : selectedNoteIndices()) {
      if (selectableNotes()[index].item == primarySelectedItem()) {
        targetIndex = index;
        break;
      }
    }
  }
  if (targetIndex == kInvalidNoteIndex && !selectedNoteIndices().empty()) {
    targetIndex = selectedNoteIndices().front();
  }
...
void PianoRollView::setPlaybackTick(int tick,
  ...
  const bool tickChanged = (tick != currentTick());
  const bool playbackStateChanged = (playbackActive != this->playbackActive());
  const bool horizontalZoomAnimating =
      m_horizontalZoomAnimation->state() == QAbstractAnimation::Running;
  if (!tickChanged && !playbackStateChanged && !activeNotes) {
    return;
  }
  const auto tickUpdate = m_playbackController.setPlaybackTick(
      tick,
      playbackActive,
      m_animClock.nsecsElapsed(),
      SequencePlayer::the().currentTempoBpm(),
      ppqn());

  updateInactiveNoteDimTarget();

  if (tickUpdate.playbackStateChanged && !this->playbackActive()) {
    stopPlaybackAutoScrollAnimation();
  ...
  if (this->playbackActive() && m_playbackAutoScrollEnabled && !horizontalZoomAnimating) {
  ...
  if (tickUpdate.playbackStateChanged || activeKeysChanged || scanlineChanged) {
...
PianoRollFrame::Data PianoRollView::captureRhiFrameData(float dpr) const {
  ...
  frame.currentTick = geometry().clampTick(currentTick());
  ...
  frame.playbackActive = playbackActive();
  ...
  frame.notes = notes();
  frame.activeNotes = m_playbackController.activeNotes();
  frame.selectedNotes = m_selectionModel.selectedNotes();
  frame.timeSignatures = timeSignatures();

  frame.activeKeyTrack.fill(-1);
  for (int key = 0; key < kMidiKeyCount; ++key) {
    frame.activeKeyTrack[static_cast<size_t>(key)] =
        m_playbackController.activeKeys()[static_cast<size_t>(key)].trackIndex;
  }
...
void PianoRollView::ensureScrollChromeButtonsInstalled() {
  if (m_scrollChromeButtonsInstalled) {
    return;
  }
...
void PianoRollView::scrollContentsBy(int dx, int dy) {
  ...
  Q_UNUSED(dy);
  if (dx != 0 && playbackActive() && !m_applyingPlaybackAutoScroll && !m_applyingHorizontalZoomScroll) {
    stopPlaybackAutoScrollAnimation();
...
void PianoRollView::timerEvent(QTimerEvent* event) {
  if (event->timerId() != m_dragAutoScrollTimer.timerId()) {
...
void PianoRollView::updateSeekDrag(int viewportX, bool forceEmit) {
  ...
  if (!forceEmit && tick == currentTick()) {
    return;
  }

  m_playbackController.setCurrentTick(tick);
  updateActiveKeyStates();
  requestRender();
  emit seekRequested(currentTick());
  updateSeekPreview();
}
...
bool PianoRollView::handleViewportCoalescedZoomGesture(float rawDelta,
  ...
  const QPoint anchor = m_rhiHost->mapFromGlobal(globalPos.toPoint());
...
bool PianoRollView::handleViewportMouseMove(QMouseEvent* event) {
  ...
  if (!contentInteractionActive && m_scrollChrome->handleMouseMove(pos, activeButtons)) {
    refreshInteractionCursor(activeModifiers);
    event->accept();
    return true;
  }
  ...
  if (!m_noteSelectionDragging && movedEnough) {
    ...
    m_noteSelectionAppendMode = activeModifiers.testFlag(Qt::ShiftModifier);
    m_noteSelectionBaseIndices = m_noteSelectionAppendMode ? selectedNoteIndices() : std::vector<size_t>{};
  }
...
bool PianoRollView::handleViewportMouseRelease(QMouseEvent* event) {
  ...
  if (!m_panDragActive && !m_seekDragActive && !m_noteSelectionPressActive &&
      m_scrollChrome->handleMouseRelease(pos)) {
    refreshInteractionCursor(activeModifiers);
    event->accept();
    return true;
  }
  ...
  if (m_noteSelectionDragging) {
    ...
    if (noteIndex >= 0) {
      std::vector<size_t> indices{static_cast<size_t>(noteIndex)};
      if (activeModifiers.testFlag(Qt::ShiftModifier)) {
        const auto& existing = selectedNoteIndices();
...
void PianoRollView::updateInactiveNoteDimTarget(bool animated) {
  const float targetAlpha = (m_lightFrameColors && (playbackActive() || m_seekDragActive))
                                ? kInactiveNoteDimMaxAlpha
                                : 0.0f;
  if (m_inactiveNoteDimAnimation->state() == QAbstractAnimation::Running &&
...
void PianoRollView::rebuildSequenceCache() {
  ...
  m_playbackController.setCurrentTick(geometry().clampTick(currentTick()));
}

bool PianoRollView::updateActiveKeyStates() {
  std::vector<PianoRollFrame::Note> resolvedActiveNotes;

  if (playbackActive() && timeline() && timelineCursor() && timeline()->finalized()) {
    std::vector<const SeqTimedEvent*> active;
    timelineCursor()->getActiveAt(static_cast<uint32_t>(std::max(0, currentTick())), active);
...
bool PianoRollView::applyResolvedActiveNotes(const std::vector<PianoRollFrame::Note>& resolvedActiveNotes) {
  return m_playbackController.applyResolvedActiveNotes(
      resolvedActiveNotes,
      [this](int trackIndex) { return isTrackEnabled(trackIndex); });
}
...
void PianoRollView::updateScrollBars() {
  ...
  m_playbackController.setCurrentTick(rollGeometry.clampTick(currentTick()));
}
...
bool PianoRollView::shouldPumpPlaybackFrames() const {
  return playbackActive() || m_frameDrivenPlaybackAutoScrollActive;
}

float PianoRollView::visualPlaybackTick() const {
  return m_playbackController.visualPlaybackTick(
      m_animClock.nsecsElapsed(),
      totalTicks(),
      SequencePlayer::the().playing());
}
...
PianoRollGeometry PianoRollView::geometry() const {
  ...
  metrics.scrollX = horizontalScrollBar()->value();
  metrics.scrollY = verticalScrollBar()->value();
...
QPoint PianoRollView::viewportPosFromGlobal(const QPointF& globalPos) const {
  return m_rhiHost->mapFromGlobal(globalPos.toPoint());
}
...
QPointF PianoRollView::autoScrollDeltaForGraphDrag(const QPoint& viewportPos) const {
  ...
  const qreal currentDpr = std::max<qreal>(1.0, viewport()->devicePixelRatioF());
...
void PianoRollView::applySelectedNoteIndices(std::vector<size_t> indices,
  ...
  if (emitSelectionSignal && (update.selectionChanged || update.primaryChanged)) {
    emitCurrentSelectionSignals();
  }
}

void PianoRollView::emitCurrentSelectionSignals() {
  emit selectionChanged(primarySelectedItem());
  std::vector<VGMItem*> selectedItems = m_selectionModel.uniqueItemsForSelectedNoteIndices(selectableNotes());
  emit selectionSetChanged(selectedItems, primarySelectedItem());
}

void PianoRollView::updateMarqueeSelection(bool emitSelectionSignal) {
  const auto applyMarqueeSelection = [this, emitSelectionSignal](std::vector<size_t> indices) {
    if (m_noteSelectionAppendMode) {
      indices.insert(indices.end(), m_noteSelectionBaseIndices.begin(), m_noteSelectionBaseIndices.end());
    }
  ...
  applyMarqueeSelection(std::move(indices));
}

void PianoRollView::updateMarqueePreview(const QPoint& cursorPos) {
  const auto& currentSelection = selectedNoteIndices();
...
void PianoRollView::applyPreviewNoteIndices(std::vector<size_t> indices, int previewTick) {
  const auto update = m_selectionModel.applyPreviewNoteIndices(
      std::move(indices),
      previewTick,
      selectableNotes());
  if (update.signal == PianoRollSelectionModel::PreviewSignal::None) {
    return;
  }
  if (update.signal == PianoRollSelectionModel::PreviewSignal::Stopped) {
    emit notePreviewStopped();
    return;
  }

  emit notePreviewRequested(update.previewNotes, update.previewTick);
}
