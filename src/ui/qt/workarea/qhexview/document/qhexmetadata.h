...
#else
using QHexLineMetadata = std::list<QHexMetadataItem>;
#endif
...
public:
  ...
  void metadata(qint64 begin, qint64 end, const QColor& fgcolor, const QColor& bgcolor,
                const QString& comment);

  // old interface with line, start, length
  void metadata(quint64 line, int start, int length, const QColor& fgcolor, const QColor& bgcolor,
                const QString& comment);
...
private:
  quint8 m_lineWidth;
  // map from line number to list of metadataitems for that line
