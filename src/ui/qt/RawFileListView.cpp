...
RawFileListViewModel::RawFileListViewModel(QObject *parent) : QAbstractListModel(parent) {
  connect(&qtVGMRoot, &QtVGMRoot::UI_AddedRawFile,
          [=]() { dataChanged(index(0, 0), index(0, 0)); });
  connect(&qtVGMRoot, &QtVGMRoot::UI_RemovedRawFile,
          [=]() { dataChanged(index(0, 0), index(0, 0)); });
}
...
RawFileListView::RawFileListView(QWidget *parent) : QListView(parent) {
