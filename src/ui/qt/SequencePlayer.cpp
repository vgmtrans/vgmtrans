...
bool SequencePlayer::syncPreviewChannelState(uint8_t channel, uint32_t tick) {
  ...
  // BASS_POS_MIDI_TICK positions to the state before that tick, so seek one tick ahead to include
  // controller/program updates that happen at the same tick as the selected note.
