...
protected:
  bool event(QEvent* event) override {
    if (event->type() == QEvent::FileOpen) {
      ...
      qtVGMRoot.openRawFileWithAccessRetry(std::filesystem::path(fileEvent->file().toStdWString()));
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < args.size(); ++i) {
    ...
    qtVGMRoot.openRawFileWithAccessRetry(std::filesystem::path(s.toStdWString()));
  }
