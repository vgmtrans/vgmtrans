		...
		virtual bool Restore(IMainState* pMState,dockwins::DFDOCKPOSEX* pDocPos)
		{
            if(pDocPos->bDocking)
            {
				...
				if(dside.IsValid())
				{
					...
					{
						pDocPos->dockPos.nWidth=static_cast<unsigned long>(pDocPos->dockPos.nWidth*pMState->YRatio());
						pDocPos->dockPos.nHeight=static_cast<unsigned long>(pDocPos->dockPos.nHeight*pMState->XRatio());
					}
					else
					{
						pDocPos->dockPos.nWidth=static_cast<unsigned long>(pDocPos->dockPos.nWidth*pMState->XRatio());
						pDocPos->dockPos.nHeight=static_cast<unsigned long>(pDocPos->dockPos.nHeight*pMState->YRatio());
					}
				}
            }
