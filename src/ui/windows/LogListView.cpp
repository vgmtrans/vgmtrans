...
void CLogListView::AddLogItem(LogItem* newLog) {
  ...
  DateTime timestamp = newLog->GetTime();
  wsprintf(timeString, L"%04d-%02d-%02d %02d:%02d:%02d.%03d", timestamp.get_Year(),
           timestamp.get_Month(), timestamp.get_Day(), timestamp.get_Hour(), timestamp.get_Minute(),
           timestamp.get_Second(), timestamp.get_Millisecond());
...
void CLogListView::InitImageLists() {
  ...
  HICON hiErr = (HICON)::LoadImage(_Module.GetResourceInstance(), MAKEINTRESOURCE(IDI_LEVEL_ERROR),
                                   IMAGE_ICON, 16, 16, LR_SHARED);
  HICON hiWarn =
      (HICON)::LoadImage(_Module.GetResourceInstance(), MAKEINTRESOURCE(IDI_LEVEL_WARNING),
                         IMAGE_ICON, 16, 16, LR_SHARED);
  HICON hiInfo = (HICON)::LoadImage(_Module.GetResourceInstance(), MAKEINTRESOURCE(IDI_LEVEL_INFO),
                                    IMAGE_ICON, 16, 16, LR_SHARED);
...
int CALLBACK CLogListView::SortCallback(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort) {
...
int CLogListView::SortCallback(const LogItem& info1, const LogItem& info2) const {
  int nRet = 0;
  LPCTSTR sz1, sz2;
