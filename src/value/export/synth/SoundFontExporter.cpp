...
[[nodiscard]] Chunk igenChunk(std::span<const ResolvedSynthInstrument> instruments,
  ...
  for (const auto& instrument : instruments) {
    ...
    for (const auto& sfRegion : instrument.regions) {
      ...
      writeAmountGen(payload, kSfGenDecayVolEnv, sf2EnvelopeTimecents(envelope.decaySeconds));
      writeAmountGen(payload, kSfGenSustainVolEnv, explicitEnvelope ? sf2SustainAttenuation(envelope) : 0);
      writeAmountGen(payload, kSfGenReleaseVolEnv, sf2EnvelopeTimecents(envelope.releaseSeconds));
