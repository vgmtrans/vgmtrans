...
void validateEnvelope(ValidationReport& report, const Envelope& envelope, SourceRange range) {
  ...
  const auto checkSeconds = [&](const std::optional<double> value, std::string_view code, std::string_view name) {
    if (value && (std::isnan(*value) || *value < 0.0)) {
      report.error(std::string(code), "Synth envelope " + std::string(name) + " time was negative or NaN", range);
    }
  };
  ...
  if (envelope.sustainAmplitude && (!std::isfinite(*envelope.sustainAmplitude) || *envelope.sustainAmplitude < 0.0 ||
    ...
    report.error("synth.envelope.sustain", "Synth envelope sustain amplitude was outside the 0.0 to 1.0 range", range);
  }
}
...
void validateSamples(ValidationReport& report, const SamplePool& pool) {
  for (const auto& sample : pool.samples) {
    if (sample.channels == 0) {
      report.error("synth.sample.channels", "Synth sample had zero channels", sample.encodedData);
    }
    if (!std::isfinite(sample.attenuationDb)) {
      report.error("synth.sample.attenuation", "Synth sample attenuation was not finite", sample.encodedData);
    }
  }
}
...
ValidationReport validateSampleReferences(const SoundBankAsset& soundBank,
  ...
  for (const auto& instrument : soundBank.instruments) {
    for (const auto& region : instrument.regions) {
      ...
      if (!sample.valid()) {
        report.error("synth.sample-reference.malformed", "Synth region has a malformed sample reference", region.range);
        continue;
      }
      ...
      if (sample.owner() != soundBank.metadata.id) {
        ...
        if (found == externalPools.end()) {
          report.error("synth.sample-reference.external-owner",
                       "Synth region sample owner is not a selected external sample pool", region.range);
          continue;
        }
        pool = &(*found)->pool;
      }
      if (sample.index() >= pool->samples.size()) {
        report.error("synth.sample-reference.out-of-range", "Synth region sample index is out of range", region.range);
      }
    }
  }
  return report;
  ...
  for (const auto& instrument : soundBank.instruments) {
    if (instrument.identity) {
      if (!instrument.identity->valid()) {
        report.error("synth.instrument.identity", "Synth instrument had an empty identity domain", instrument.range);
    ...
    for (const auto& region : instrument.regions) {
      ...
      if (!std::isfinite(region.pan) || region.pan < 0.0 || region.pan > 1.0) {
        report.error("synth.region.pan", "Synth region pan was outside the 0.0 to 1.0 range", region.range);
      }
      if (!std::isfinite(region.attenuationDb)) {
        report.error("synth.region.attenuation", "Synth region attenuation was not finite", region.range);
      }
      if (!std::isfinite(region.unityKey)) {
        report.error("synth.region.unity-key", "Synth region unity key was not finite", region.range);
      }
