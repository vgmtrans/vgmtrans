...
[[nodiscard]] NdsBytecodeMap ndsBytecodeMap(Registrar& registrar) {
  ...
  map.template range<0x00, 0x7f, Note>("Note");
  map.template op<0x80, Rest>("Rest");
  map.template op<0x81, Program>("Program");
  map.preserved(0x93, "Open Track", bytes(4));
  map.template op<0x94, Jump>("Jump");
  map.template op<0x95, Call>("Call");
  map.template terminal<0x96, Terminal>("Terminal");
  ...
  map.template op<0xc0, Pan>("Pan");
  map.template op<0xc1, Volume>("Volume");
  map.preserved(0xc2, "Master Volume", bytes(1));
  map.template op<0xc3, Transpose>("Transpose");
  map.template op<0xc4, PitchBend>("Pitch Bend");
  map.template op<0xc5, PitchBendRange>("Pitch Bend Range");
  map.preserved(0xc6, "Priority", bytes(1));
  map.template op<0xc7, NoteWait>("Note Wait");
  map.preserved(0xc8, "Tie", bytes(1));
  map.preserved(0xc9, "Portamento Control", bytes(1));
  map.template op<0xca, ModulationDepth>("Modulation Depth");
  ...
  map.template op<0xce, PortamentoSwitch>("Portamento");
  map.template op<0xcf, PortamentoTime>("Portamento Time");
  ...
  map.template op<0xd5, ExpressionLevel>("Expression");
  map.preserved(0xd6, "Print Variable", bytes(1));
  map.preserved(0xe0, "Modulation Delay", bytes(2));
  map.template op<0xe1, Tempo>("Tempo");
  map.preserved(0xe3, "Sweep Pitch", bytes(2));
  map.preserved(0xfc, "Loop End");
  map.template returns<0xfd, Return>("Return");
  map.preserved(0xfe, "Allocate Track", bytes(2));
  map.template terminal<0xff, End>("End");
  map.template unknown<Terminal>("Terminal");
