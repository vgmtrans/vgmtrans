...
[[nodiscard]] NormalStreamAnalysis analyzeNormalStream(ByteReader reader, u32 start, u32 end) {
  std::set<u8> banks;
  u32 offset = start;
  u32 commands = 0;
  while (offset < end) {
  ...
  return NormalStreamAnalysis{
      .end = std::min(offset, end),
      .referencedBanks = {banks.begin(), banks.end()},
  };
}
...
std::vector<SegSatBankLayout> findSegSatBanks(ByteReader reader) {
  ...
  for (u32 base = 0; base + 10 <= end;) {
    auto layout = readSegSatBankLayout(reader, base);
    if (!layout) {
      ++base;
      continue;
    }
...
std::vector<SegSatSequenceLayout> findSegSatSequences(ByteReader reader) {
  ...
  for (u32 table = 0; table + 0x20 < fileEnd; ++table) {
    ...
    for (u32 index = 0; index < sequenceCount; ++index) {
      ...
      const auto normalStream = analyzeNormalStream(reader, offset + normal, sequenceEnd);
      sequences.push_back(SegSatSequenceLayout{
          ...
          .normalTrack = normal,
          .normalTrackEnd = normalStream.end,
          .tempoLoop = tempoLoop,
          .referencedBanks = normalStream.referencedBanks,
      });
      ++accepted;
    }
    if (!valid) {
