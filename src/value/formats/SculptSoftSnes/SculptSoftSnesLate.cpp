...
DecodedBytecodeCommand decodeCommand(ByteReader reader, u32 offset, Address start, bool& fine, const Layout& layout,
  ...
  if (fine) {
    ...
    if (opcode == 1) {
      const u8 duration = cursor.u8("duration (0 = 256)");
      return event.invokeFlow<&Playback::timed>({TimedEvent::Wait, s16{0}, u16(duration == 0 ? 256 : duration)});
    }
    const s16 delta = opcode < 16 ? cursor.s8("pitch delta (1/20 semitone)") : s16((opcode >> 4) - 8);
  ...
  if (opcode < 0xe0) {
    ...
    if (opcode < 0x80) {
      return event.invokeFlow<&Playback::timed>(
          {TimedEvent::RelativeNote, s16((opcode >> 3) - 8), u16((opcode & 7) + 1)});
    }
    const u8 duration = cursor.u8("duration (0 = 256)");
    return event.invokeFlow<&Playback::timed>(
        {TimedEvent::AbsoluteNote, s16(opcode - 0x60), u16(duration == 0 ? 256 : duration)});
  }
  switch (opcode) {
    case 0xe0:
      return cursor.command("DSP Pitch Offset", SequenceSemantic::Pitch)
          .invoke<&Playback::parameter>({opcode, cursor.u16le("offset")});
    case 0xf7:
      return cursor.command("Track Pitch Offset", SequenceSemantic::Pitch)
          .invoke<&Playback::transpose>({cursor.u16le("offset (1/20 semitone)"), layout.sharedTranspose});
    case 0xe1:
    case 0xe2:
    case 0xe3:
    case 0xe4: {
      ...
      return cursor.command(labels[opcode - 0xe1], SequenceSemantic::Envelope)
          .invoke<&Playback::parameter>({opcode, u16(cursor.u8("multiplier / 32 (0 = default)"))});
    }
    case 0xe5: {
      auto event = cursor.command("Voice Allocation", SequenceSemantic::State);
      const u8 mode = cursor.u8("mode");
      const u8 voice = cursor.u8("starting voice");
      return event.invoke<&Playback::assign>({mode, voice, cursor.u8("voice mask")});
    }
    case 0xe6:
    ...
    case 0xed:
      return cursor.command("Volume Multiplier", SequenceSemantic::Level)
          .invoke<&Playback::parameter>({opcode, u16(cursor.u8("multiplier / 256"))});
    case 0xf3:
      return cursor.command("Voice Priority", SequenceSemantic::State)
          .invoke<&Playback::parameter>({opcode, u16(cursor.u8("priority"))});
    case 0xfa:
      return cursor.command("Tempo", SequenceSemantic::Tempo)
          .invoke<&Playback::parameter>({opcode, u16(cursor.u8("tempo accumulator step"))});
    case 0xfb:
      return cursor.command("Pan Offset", SequenceSemantic::Pan)
          .invoke<&Playback::parameter>({opcode, static_cast<u16>(cursor.s8("offset"))});
    case 0xef: {
      auto event = cursor.command("Absolute Note", SequenceSemantic::Note);
      const u8 note = cursor.u8("note");
      const u8 duration = cursor.u8("duration (0 = 256)");
      return event.invokeFlow<&Playback::timed>(
          {TimedEvent::AbsoluteNote, s16(note), u16(duration == 0 ? 256 : duration)});
    }
    case 0xf0:
