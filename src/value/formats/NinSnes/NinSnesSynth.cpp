...
[[nodiscard]] bool validHeader(ByteReader reader, const Profile& selected, const InstrumentInfo& info,
  ...
  if (info.srcn >= 0x80 && selected.intelli != IntelliMode::Fe4 && selected.id != ProfileId::QuestTacticsOgre) {
    return false;
  }
...
[[nodiscard]] std::vector<InstrumentInfo> collectBaseInstruments(ByteReader reader, const Layout& layout) {
  ...
  // Ogre Battle resolves banked SRCNs and built-in rows into instrument recipes.
  if (!layout.instrumentTableAddress || !layout.spcDirAddress || layout.profile == ProfileId::QuestOgreBattle) {
    return infos;
  }
...
void applyPitchWrap(Region& region, u16 pitchScale, int key, bool questOgreBattle = false) {
  ...
  const int correction = questOgreBattle ? 0 : note >= 0x34 ? note - 0x34 : note < 0x13 ? (note - 0x13) * 2 : 0;
    ...
    if (noise) {
      ...
      for (int key = 0; key < 128; ++key) {
        ...
        if (recipes.usedNotes.contains({info.program, static_cast<u8>(key)})) {
          applyPitchWrap(tuned, pitchScale, key, selected.id == ProfileId::QuestOgreBattle);
        }
...
std::optional<ScanSoundBankDraft> addSynth(ScanResultBuilder& builder, const Layout& layout,
  ...
  std::vector<InstrumentInfo> overrides = collectOverrides(recipes, layout.profile != ProfileId::QuestOgreBattle);
