...
namespace vgmtrans::formats::hosa::bytecode {
...
[[nodiscard]] inline std::optional<u32> variableEnd(core::ByteReader reader, u32 offset, u32 end) {
  if (offset >= end || !reader.has(offset, 1)) return std::nullopt;
...
[[nodiscard]] inline std::optional<u32> commandEnd(core::ByteReader reader, u32 offset, u32 end) {
  ...
  if (status < 0x80) {
    ...
    const bool restoresLoop = (status & 0x1f) == 9 && offset < end && reader.u8At(offset) != 0;
    offset += kControlParameterBytes[status & 0x1f];
    // A nonzero loop command restores the saved stream pointers immediately;
    // its encoded timing mode is never consumed by the driver.
    if (restoresLoop) return offset <= end ? std::optional<u32>{offset} : std::nullopt;
    if ((status & 0x60) == 0x40) {
      const auto next = variableEnd(reader, offset, end);
      if (!next) return std::nullopt;
      offset = *next;
