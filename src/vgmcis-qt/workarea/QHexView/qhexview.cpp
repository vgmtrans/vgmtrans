...
QHexView::QHexView(QWidget *parent)
    : QAbstractScrollArea(parent), m_document(nullptr), m_renderer(nullptr), m_readonly(false) {
...
void QHexView::moveNext(bool select) {
    ...
    bool lastcell =
        (line >= m_renderer->documentLastLine()) && (column >= m_renderer->documentLastColumn());
...
bool QHexView::processMove(QHexCursor *cur, QKeyEvent *e) {
    if (e->matches(QKeySequence::MoveToNextChar) || e->matches(QKeySequence::SelectNextChar))
        ...
        int pageline =
            std::min(m_renderer->documentLastLine(), cur->currentLine() + this->visibleLines());
...
int QHexView::firstVisibleLine() const {
    return this->verticalScrollBar()->value();
}
int QHexView::lastVisibleLine() const {
