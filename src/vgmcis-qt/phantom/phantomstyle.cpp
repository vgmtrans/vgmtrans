...
void PhantomStyle::drawControl(ControlElement element,
  ...
  case CE_DockWidgetTitle: {
    ...
    if (verticalTitleBar) {
      QRect r = dwOpt->rect;
      QRect rcis = r.transposed();
      titleRect = QRect(rcis.left() + r.bottom() - titleRect.bottom(),
                        rcis.top() + titleRect.left() - r.left(),
                        titleRect.height(), titleRect.width());
      painter->translate(rcis.left(), rcis.top() + rcis.width());
      painter->rotate(-90);
      painter->translate(-rcis.left(), -rcis.top());
    }
