...
/*
 This is a modified version of the fluid_midi.h file from version FluidSynth 1.1.6.
...
enum vgmtrans_fluid_midi_event_type {
    /* channel messages */
            NOTE_OFF = 0x80,
    NOTE_ON = 0x90,
    KEY_PRESSURE = 0xa0,
...
struct _vgmtrans_fluid_midi_event_t {
    ...
    void *track;              /* Pointer to parent track, potentially useful for event callback purposes. */
};
...
struct _fluid_midi_parser_t {
    ...
    vgmtrans_fluid_midi_event_t event;        /* The event, that is returned to the MIDI driver. */
};
