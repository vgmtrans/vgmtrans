...
void VGMSeq::LoadTracksMain(long stopTime) {
    ...
    if (bLoadTickByTick) {
        while (HasActiveTracks()) {
            ...
            // check loop count
            int requiredLoops =
                (readMode == READMODE_ADD_TO_UI) ? 1 : ConversionOptions::GetNumSequenceLoops();
...
void VGMSeq::AddInstrumentRef(uint32_t progNum) {
    if (std::find(aInstrumentsUsed.begin(), aInstrumentsUsed.end(), progNum) ==
        aInstrumentsUsed.end()) {
