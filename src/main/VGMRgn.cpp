...
void VGMRgn::AddKeyLow(uint8_t kl, uint32_t offset, uint32_t length) {
    keyLow = kl;
    items.push_back(
        new VGMRgnItem(this, VGMRgnItem::RIT_KEYLOW, offset, length, L"Note Range: Low Key"));
}

void VGMRgn::AddKeyHigh(uint8_t kh, uint32_t offset, uint32_t length) {
    keyHigh = kh;
    items.push_back(
        new VGMRgnItem(this, VGMRgnItem::RIT_KEYHIGH, offset, length, L"Note Range: High Key"));
}

void VGMRgn::AddVelLow(uint8_t vl, uint32_t offset, uint32_t length) {
    velLow = vl;
    items.push_back(
        new VGMRgnItem(this, VGMRgnItem::RIT_VELLOW, offset, length, L"Vel Range: Low"));
}

void VGMRgn::AddVelHigh(uint8_t vh, uint32_t offset, uint32_t length) {
    velHigh = vh;
    items.push_back(
        new VGMRgnItem(this, VGMRgnItem::RIT_VELHIGH, offset, length, L"Vel Range: High"));
}

void VGMRgn::AddSampNum(int sn, uint32_t offset, uint32_t length) {
    sampNum = sn;
    items.push_back(
        new VGMRgnItem(this, VGMRgnItem::RIT_SAMPNUM, offset, length, L"Sample Number"));
}
