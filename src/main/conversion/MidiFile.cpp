...
void MidiTrack::WriteTrack(std::vector<uint8_t> &buf) {
    ...
    stable_sort(finalEvents.begin(), finalEvents.end(),
                PriorityCmp());  // Sort all the events by priority
    stable_sort(finalEvents.begin(), finalEvents.end(),
                AbsTimeCmp());  // Sort all the events by absolute time, so that delta times can be
                                // recorded correctly
    ...
    uint32_t trackSize =
        (uint32_t)(buf.size() - 8);  //-8 for MTrk and size that shouldn't be accounted for
...
void MidiTrack::AddNoteByDur_TriAce(uint8_t channel, int8_t key, int8_t vel, uint32_t duration) {
    ...
    for (size_t curEvt = 0; curEvt < nNumEvents; curEvt++) {
        ...
        // If so, we're restarting an already played note. In the case of the TriAce driver that
        // means, that we resume the note, so we need to move the NoteOff event.
        ...
        //       Valkyrie Profile: 225 Fragments of the Heart confirms, that this is NOT the case in
        //       the PS1 version.
        if (aEvents[curEvt]->AbsTime > CurDelta &&
            aEvents[curEvt]->GetEventType() == MIDIEVENT_NOTEON) {
    ...
    if (ContNote == NULL) {
        ...
        ContNote->AbsTime =
            CurDelta + duration;  // fix DeltaTime of the already inserted NoteOff event
    }
}

void MidiTrack::InsertNoteByDur(uint8_t channel, int8_t key, int8_t vel, uint32_t duration,
                                uint32_t absTime) {
...
void MidiTrack::InsertControllerEvent(uint8_t channel, uint8_t controllerNum, uint8_t theDataByte,
                                      uint32_t absTime) {
...
void MidiTrack::InsertPitchBendRange(uint8_t channel, uint8_t semitones, uint8_t cents,
                                     uint32_t absTime) {
    ...
    aEvents.push_back(
        new ControllerEvent(this, channel, absTime, 6, semitones, PRIORITY_HIGHER - 1));
...
void MidiTrack::InsertModulationDepthRange(uint8_t channel, uint8_t msb, uint8_t lsb,
                                           uint32_t absTime) {
    aEvents.push_back(new ControllerEvent(this, channel, absTime, 101, 0, PRIORITY_HIGHER - 1));
...
void MidiTrack::InsertTimeSig(uint8_t numer, uint8_t denom, uint8_t ticksPerQuarter,
                              uint32_t absTime) {
...
MidiEvent::MidiEvent(MidiTrack *thePrntTrk, uint32_t absoluteTime, uint8_t theChannel,
                     int8_t thePriority)
...
NoteEvent::NoteEvent(MidiTrack *prntTrk, uint8_t channel, uint32_t absoluteTime, bool bnoteDown,
                     uint8_t theKey, uint8_t theVel)
...
{
        prntTrk->aEvents.push_back(new NoteEvent(prntTrk, channel, AbsTime, true, key, vel));
//add note on prntTrk->aEvents.push_back(new NoteEvent(prntTrk, channel, AbsTime+duration, false,
...
/*VolEvent::VolEvent(MidiTrack *prntTrk, uint8_t channel, uint32_t absoluteTime, uint8_t theVol,
int8_t thePriority) : ControllerEvent(prntTrk, channel, absoluteTime, 7), vol(theVol)
{
}
...
MastVolEvent::MastVolEvent(MidiTrack *prntTrk, uint8_t channel, uint32_t absoluteTime,
                           uint8_t theMastVol)
...
ControllerEvent::ControllerEvent(MidiTrack *prntTrk, uint8_t channel, uint32_t absoluteTime,
                                 uint8_t controllerNum, uint8_t theDataByte, int8_t thePriority)
...
ProgChangeEvent::ProgChangeEvent(MidiTrack *prntTrk, uint8_t channel, uint32_t absoluteTime,
                                 uint8_t progNum)
...
PitchBendEvent::PitchBendEvent(MidiTrack *prntTrk, uint8_t channel, uint32_t absoluteTime,
                               int16_t bendAmt)
...
TimeSigEvent::TimeSigEvent(MidiTrack *prntTrk, uint32_t absoluteTime, uint8_t numerator,
                           uint8_t denominator, uint8_t clicksPerQuarter)
