...
MP2kInstrSet::MP2kInstrSet(RawFile *file, int rate, size_t offset, int count, std::string name)
    : VGMInstrSet(MP2kFormat::name, file, gsl::narrow_cast<u32>(offset),
                  gsl::narrow_cast<u32>(count * 12), name),
      m_count(count),
...
int MP2kInstrSet::MakeOrGetSample(size_t sample_pointer) {
    ...
    samp->volume = 1;
    samp->bps = 8;
    samp->sampName = fmt::format("{:#x}", sample_pointer);
...
MP2kInstr::MP2kInstr(MP2kInstrSet *set, size_t offset, size_t length, u32 bank, u32 number,
                     MP2kInstrData data)
    : VGMInstr(set, gsl::narrow_cast<u32>(offset), gsl::narrow_cast<u32>(length), bank, number),
      m_type(data.w0 & 0xFF),
      m_data(data) {}
        ...
        case 0x40: {
            ...
            for (int i = 0; i < index_list.size(); i++) {
                ...
                if (auto sample_id =
                    ...
                    sample_id != -1) {
                    VGMRgn *rgn =
                        AddRgn(dwOffset, unLength, sample_id, split_list[i], split_list[i + 1] - 1);
        ...
        case 0x80: {
            ...
            for (int key = 0; key < 128; key++) {
                ...
                if ((type & 0x0f) == 0 || (type & 0x0f) == 8) {
                    if (auto sample_id = static_cast<MP2kInstrSet *>(parInstrSet)
                                             ->MakeOrGetSample(sample_pointer);
                        sample_id != -1) {
                        ...
                        double delta_note =
                            12.0 * log2(static_cast<MP2kInstrSet *>(parInstrSet)->sampleRate() *
                                        1024.0 / pitch);
