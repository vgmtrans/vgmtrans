...
SoftCreatSnesSeq::SoftCreatSnesSeq(RawFile *file, SoftCreatSnesVersion ver, uint32_t seqdataOffset,
                                   uint8_t headerAlignSize, std::wstring newName)
    : VGMSeq(SoftCreatSnesFormat::name, file, seqdataOffset, 0, newName),
      version(ver),
...
bool SoftCreatSnesTrack::ReadEvent(void) {
    ...
    std::map<uint8_t, SoftCreatSnesSeqEventType>::iterator pEventType =
    ...
    switch (eventType) {
        ...
        default:
            ...
            pRoot->AddLogItem(new LogItem((std::wstring(L"Unknown Event - ") + desc.str()).c_str(),
                                          LOG_LEVEL_ERR, L"SoftCreatSnesSeq"));
            bContinue = false;
            break;
    }
