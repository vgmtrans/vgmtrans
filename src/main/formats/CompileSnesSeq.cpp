...
const uint8_t CompileSnesSeq::noteDurTable[] = {
    0x01, 0x02, 0x03, 0x04, 0x06, 0x08, 0x0c, 0x10, 0x18, 0x20, 0x30, 0x09, 0x12, 0x1e, 0x24, 0x2a,
};

CompileSnesSeq::CompileSnesSeq(RawFile *file, CompileSnesVersion ver, uint32_t seqdataOffset,
                               std::wstring newName)
    : VGMSeq(CompileSnesFormat::name, file, seqdataOffset),
      version(ver),
...
void CompileSnesSeq::LoadEventMap() {
    ...
    for (unsigned int statusByte = STATUS_PERCUSSION_NOTE_MIN;
         statusByte <= STATUS_PERCUSSION_NOTE_MAX; statusByte++) {
        EventMap[statusByte] = EVENT_PERCUSSION_NOTE;
    }

    EventMap[STATUS_DURATION_DIRECT] = EVENT_DURATION_DIRECT;
    for (unsigned int statusByte = STATUS_DURATION_MIN; statusByte <= STATUS_DURATION_MAX;
         statusByte++) {
...
void CompileSnesTrack::AddInitialMidiEvents(int trackNum) {
    ...
    AddPanNoItem(
        Convert7bitLinearPercentPanValToStdMidiVal((uint8_t)(spcPan + 0x80) / 2, &volumeScale));
...
bool CompileSnesTrack::ReadEvent(void) {
    ...
    std::map<uint8_t, CompileSnesSeqEventType>::iterator pEventType =
    ...
    switch (eventType) {
        ...
        case EVENT_LOOP_END: {
            ...
            desc << L"Nest Level: " << (int)repeatNest << L"  Destination: $" << std::hex
                 << std::setfill(L'0') << std::setw(4) << std::uppercase << (int)dest;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Repeat End", desc.str().c_str(),
        ...
        case EVENT_LOOP_COUNT: {
            ...
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Loop Count", desc.str().c_str(),
        ...
        case EVENT_FLAGS: {
            ...
            spcFlags = flags;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Flags", desc.str().c_str(),
        ...
        case EVENT_TUNING: {
            int16_t newTuning;
            if (parentSeq->version == COMPILESNES_ALESTE ||
                parentSeq->version == COMPILESNES_JAKICRUSH) {
        ...
        case EVENT_PORTAMENTO_ON: {
            ...
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Portamento On",
        ...
        case EVENT_PORTAMENTO_OFF: {
            spcFlags &= ~COMPILESNES_FLAGS_PORTAMENTO;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Portamento Off",
                            desc.str().c_str(), CLR_PORTAMENTO, ICON_CONTROL);
            break;
        }
        ...
        case EVENT_PANPOT_ENVELOPE: {
            uint8_t envelopeIndex = GetByte(curOffset++);
            desc << L"Envelope Index: " << (int)envelopeIndex;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Panpot Envelope",
        ...
        case EVENT_PAN: {
            ...
            double volumeScale;
            uint8_t midiPan = Convert7bitLinearPercentPanValToStdMidiVal(
                (uint8_t)(newPan + 0x80) / 2, &volumeScale);
            AddExpressionNoItem(ConvertPercentAmpToStdMidiVal(volumeScale));
        ...
        case EVENT_LOOP_BREAK: {
            ...
            desc << L"Nest Level: " << (int)repeatNest << L"  Destination: $" << std::hex
                 << std::setfill(L'0') << std::setw(4) << std::uppercase << (int)dest;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Repeat Break",
        ...
        case EVENT_DURATION_DIRECT: {
            ...
            desc << L"Duration: " << (int)duration;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Duration (Direct)",
        ...
        case EVENT_DURATION: {
            ...
            desc << L"Duration: " << (int)duration;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Duration", desc.str().c_str(),
        ...
        case EVENT_PERCUSSION_NOTE: {
            ...
            AddPercNoteByDur(beginOffset, curOffset - beginOffset, percNoteNumber, 100,
                             spcNoteDuration,
        ...
        default:
            ...
            pRoot->AddLogItem(new LogItem((std::wstring(L"Unknown Event - ") + desc.str()).c_str(),
                                          LOG_LEVEL_ERR, L"CompileSnesSeq"));
            bContinue = false;
            break;
    }
...
bool CompileSnesTrack::ReadDurationBytes(uint32_t &offset, uint8_t &duration) {
    ...
    while (curOffset < 0x10000) {
        ...
        } else if (statusByte >= parentSeq->STATUS_DURATION_MIN &&
                   statusByte <= parentSeq->STATUS_DURATION_MAX) {
