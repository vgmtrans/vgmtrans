...
bool TriAcePS1InstrSet::GetHeaderInfo() {
  ...
  header->addSimpleChild(dwOffset, 4, "InstrSet Size");
  header->addSimpleChild(dwOffset + 4, 2, "Instr Section Size");
...
bool TriAcePS1InstrSet::GetInstrPointers() {
  ...
  for (uint32_t i = dwOffset + sizeof(TriAcePS1InstrSet::_InstrHeader);                    //1,Sep.2009 revise
    ...
    newInstr->addSimpleChild(i + 0, sizeof(short), "Instrument Number");            //1,Sep.2009 revise
    newInstr->addSimpleChild(i + 2, sizeof(short), "ADSR1");                        //1,Sep.2009 revise
    newInstr->addSimpleChild(i + 4, sizeof(short), "ADSR2");                        //1,Sep.2009 revise
    newInstr->addUnknownChild(i + 6, sizeof(char));                                  //1,Sep.2009 revise
    newInstr->addSimpleChild(i + 7, sizeof(char), "Number of Rgns");                //1,Sep.2009 revise
...
//		this->bank++;
  ...
  for (int i = 0; i < instrinfo.numRgns; i++) {
    ...
    rgn->addSimpleChild(rgn->dwOffset + 15, 5, "Unknown values");
