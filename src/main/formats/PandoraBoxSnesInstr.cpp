...
PandoraBoxSnesInstrSet::PandoraBoxSnesInstrSet(RawFile *file, PandoraBoxSnesVersion ver,
...
bool PandoraBoxSnesInstrSet::GetInstrPointers() {
    ...
    for (uint8_t instrNum = 0; instrNum <= 0x7f; instrNum++) {
        ...
        auto iterInstrItem =
            std::find(globalInstrTable.begin(), globalInstrTable.end(), globalInstrNum);
    ...
    SNESSampColl *newSampColl =
        new SNESSampColl(PandoraBoxSnesFormat::name, this->rawfile, spcDirAddr, usedSRCNs);
...
PandoraBoxSnesRgn::PandoraBoxSnesRgn(PandoraBoxSnesInstr *instr, PandoraBoxSnesVersion ver,
