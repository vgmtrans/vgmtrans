...
static unsigned const SDSP_COUNTER_RATES[32] = {
    0x7800,  // never fires
    2048,   1536, 1280, 1024, 768, 640, 512, 384, 320, 256, 192, 160, 128, 96, 80, 64,
    48,     40,   32,   24,   20,  16,  12,  10,  8,   6,   5,   4,   3,   2,  1};
...
void SNESConvADSR(T *rgn, uint8_t adsr1, uint8_t adsr2, uint8_t gain) {
    ...
    if (adsr_enabled) {
        ...
        ConvertSNESADSR(adsr1, adsr2, gain, 0x7ff, &rgn->attack_time, &rgn->decay_time,
                        &rgn->sustain_level, &rgn->sustain_time, &rgn->release_time);
        ...
        if (sl == 7) {
            ...
            rgn->decay_time =
                (rgn->decay_time * decayTimeRate) + (rgn->sustain_time * (1.0 - decayTimeRate));
...
class SNESSampColl : public VGMSampColl {
   public:
    SNESSampColl(const std::string &format, RawFile *rawfile, uint32_t offset,
                 uint32_t maxNumSamps = 256);
    SNESSampColl(const std::string &format, VGMInstrSet *instrset, uint32_t offset,
                 uint32_t maxNumSamps = 256);
    SNESSampColl(const std::string &format, RawFile *rawfile, uint32_t offset,
                 const std::vector<uint8_t> &targetSRCNs, std::wstring name = L"SNESSampColl");
    SNESSampColl(const std::string &format, VGMInstrSet *instrset, uint32_t offset,
                 const std::vector<uint8_t> &targetSRCNs, std::wstring name = L"SNESSampColl");
