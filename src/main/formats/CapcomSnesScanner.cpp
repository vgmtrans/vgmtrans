...
BytePattern CapcomSnesScanner::ptnReadSongList("\x1c\x5d\xf5\x03\x0e\xc4\xc0\xf5"
...
// 05ac: 8d 00     mov   y,#$00
// 05ae: dd        mov   a,y
BytePattern CapcomSnesScanner::ptnReadBGMAddress("\x6f\x3f\xef\x06\x8f\x0d\xa1\x8f"
...
BytePattern CapcomSnesScanner::ptnDspRegInit("\x8d\x03\xf6\x63\x04\xc5\xf2\x00"
...
// 0b34: c8 0c     cmp   x,#$0c
// 0b36: d0 f1     bne   $0b29
BytePattern CapcomSnesScanner::ptnDspRegInitOldVer("\xf5\xf9\x0b\xfd\xf5\x05\x0c\x3f"
...
BytePattern CapcomSnesScanner::ptnLoadInstrTableAddress("\x8d\x06\xcf\xda\xa0\x60\x98\xac"
                                                        "\xa0\x98\x47\xa1",
                                                        "xxxx?xx?"
                                                        "?x??",
                                                        12);
...
void CapcomSnesScanner::SearchForCapcomSnesFromARAM(RawFile *file) {
    ...
    wstring basefilename = RawFile::removeExtFromPath(file->name());
    ...
    if (hasSongList) {
        addrSongList = std::min(file->GetShort(ofsReadSongListASM + 3),
                                file->GetShort(ofsReadSongListASM + 8));
    }

    // find BGM address
    bgmAtFixedAddress = file->SearchBytePattern(ptnReadBGMAddress, ofsReadBGMAddressASM);
    if (bgmAtFixedAddress) {
        addrBGMHeader = (file->GetByte(ofsReadBGMAddressASM + 5) << 8) |
                        file->GetByte(ofsReadBGMAddressASM + 8);
    }
    ...
    if (hasSongList) {
        if (bgmAtFixedAddress) {
            ...
            // - Captain Commando
            bool bgmHeaderCoversSongList =
                (addrBGMHeader <= addrSongList && addrBGMHeader + 17 > addrSongList);
    ...
    if (hasSongList) {
        ...
        if (loadOnlyCurrentSong) {
            ...
            if (guessedSongIndex != -1) {
                ...
                CapcomSnesSeq *newSeq =
                    new CapcomSnesSeq(file, version, addrSongHeader, true, name);
                if (!newSeq->LoadVGMFile()) {
                    delete newSeq;
                    return;
                }
            ...
            for (int songIndex = 0; songIndex < songListLength; songIndex++) {
                ...
                CapcomSnesSeq *newSeq =
                    new CapcomSnesSeq(file, version, addrSongHeader, true,
...
uint16_t CapcomSnesScanner::GetCurrentPlayAddressFromARAM(RawFile *file, CapcomSnesVersion version,
                                                          uint8_t channel) {
    uint16_t currentAddress;
    if (version == CAPCOMSNES_V1_BGM_IN_LIST) {
        currentAddress =
            file->GetByte(0x00 + channel * 2 + 1) | (file->GetByte(0x10 + channel * 2 + 1) << 8);
    } else {
...
int8_t CapcomSnesScanner::GuessCurrentSongFromARAM(RawFile *file, CapcomSnesVersion version,
