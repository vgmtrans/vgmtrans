...
bool KonamiSnesInstrSet::GetInstrPointers() {
    ...
    for (int instr = 0; instr <= 0xff; instr++) {
        ...
        if (!KonamiSnesInstr::IsValidHeader(this->rawfile, version, addrInstrHeader, spcDirAddr,
                                            false)) {
            if (instr < firstBankedInstr) {
                continue;
            } else {
    ...
    SNESSampColl *newSampColl =
        new SNESSampColl(KonamiSnesFormat::name, this->rawfile, spcDirAddr, usedSRCNs);
...
bool KonamiSnesInstr::IsValidHeader(RawFile *file, KonamiSnesVersion version,
...
KonamiSnesRgn::KonamiSnesRgn(KonamiSnesInstr *instr, KonamiSnesVersion ver, uint32_t offset,
                             bool percussion)
