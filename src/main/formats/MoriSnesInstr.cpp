...
bool MoriSnesInstrSet::GetInstrPointers() {
    ...
    for (uint8_t instrNum = 0; instrNum < instrumentAddresses.size(); instrNum++) {
        ...
        if (!instrumentHints[instrAddress].percussion) {
            ...
            for (uint8_t percNoteKey = 0;
                 percNoteKey < instrumentHints[instrAddress].percHints.size(); percNoteKey++) {
                MoriSnesInstrHint *instrHint =
                    &instrumentHints[instrAddress].percHints[percNoteKey];
                if (instrHint->startAddress < instrStartAddress) {
                    instrStartAddress = instrHint->startAddress;
                }
                if (instrHint->startAddress + instrHint->size > instrEndAddress) {
    ...
    SNESSampColl *newSampColl =
        new SNESSampColl(MoriSnesFormat::name, this->rawfile, spcDirAddr, usedSRCNs);
...
MoriSnesRgn::MoriSnesRgn(MoriSnesInstr *instr, MoriSnesVersion ver, uint32_t spcDirAddr,
                         const MoriSnesInstrHint &instrHint, int8_t percNoteKey)
    : VGMRgn(instr, instrHint.rgnAddress, 7), version(ver) {
    uint16_t rgnAddress = instrHint.rgnAddress;
