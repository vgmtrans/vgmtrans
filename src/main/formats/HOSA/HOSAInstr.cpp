...
bool HOSAInstrSet::GetHeaderInfo() {
  ...
  wdsHeader->addSimpleChild(dwOffset + 8, sizeof(uint32_t), "Number of Instruments");
...
bool HOSAInstr::LoadInstr() {
  ...
  addSimpleChild(dwOffset, sizeof(uint32_t), "Number of Rgns");
  ...
  for (unsigned int i = 0; i < instrinfo.numRgns; i++) {
    ...
    rgn->addSimpleChild(rgn->dwOffset, 4, "Sample Offset");
    ...
    rgn->addSimpleChild(rgn->dwOffset + 0x07, 1, "Semi Tone Tune");
    ...
    rgn->addSimpleChild(rgn->dwOffset + 0x0C, 4, "ADSR Values (non-standard ordering)");
