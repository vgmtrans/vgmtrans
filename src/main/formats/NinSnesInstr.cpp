...
NinSnesInstrSet::NinSnesInstrSet(RawFile *file, NinSnesVersion ver, uint32_t offset,
...
bool NinSnesInstrSet::GetInstrPointers() {
    ...
    for (int instr = 0; instr <= instr_max; instr++) {
        ...
        if (version != NINSNES_EARLIER) {
            if (GetByte(addrInstrHeader) == 0xff && GetByte(addrInstrHeader + 1) == 0xff &&
                GetByte(addrInstrHeader + 2) == 0xff && GetByte(addrInstrHeader + 3) == 0xff &&
                GetByte(addrInstrHeader + 4) == 0xff && GetByte(addrInstrHeader + 5) == 0xff) {
                continue;
            }
        }
    ...
    SNESSampColl *newSampColl =
        new SNESSampColl(NinSnesFormat::name, this->rawfile, spcDirAddr, usedSRCNs);
...
NinSnesInstr::NinSnesInstr(VGMInstrSet *instrSet, NinSnesVersion ver, uint32_t offset,
                           uint32_t theBank, uint32_t theInstrNum, uint32_t spcDirAddr,
                           const std::string &name)
    : VGMInstr(instrSet, offset, NinSnesInstr::ExpectedSize(ver), theBank, theInstrNum, name),
      version(ver),
      spcDirAddr(spcDirAddr),
      konamiTuningTableAddress(0),
      konamiTuningTableSize(0) {}
...
bool NinSnesInstr::LoadInstr() {
    ...
    NinSnesRgn *rgn =
        new NinSnesRgn(this, version, dwOffset, konamiTuningTableAddress, konamiTuningTableSize);
...
bool NinSnesInstr::IsValidHeader(RawFile *file, NinSnesVersion version, uint32_t addrInstrHeader,
...
NinSnesRgn::NinSnesRgn(NinSnesInstr *instr, NinSnesVersion ver, uint32_t offset,
                       uint16_t konamiTuningTableAddress, uint8_t konamiTuningTableSize)
