        ...
        for (uint32_t j = 0; j < rgns->size(); j++) {
            ...
            if (art->loop_point != 0)
                rgn->SetLoopInfo(1, art->loop_point,
                                 sampcoll->samples[rgn->sampNum]->dataLength - art->loop_point);
...
bool AkaoColl::PreDLSMainCreation() {
    ...
    // in the Akao Sample Collection, so that they can be used with the 0xA1 program change sequence
    // event to do this, we create a copy of the AkaoInstrSet before conversion and add the
    // instruments, then when we finish the dls conversion, we put back the original unchanged
    ...
    *newInstrSet = *origInstrSet;
    ...
    for (uint32_t i = 0; i < numAddedInstrs; i++) {
        ...
        AkaoInstr *newInstr =
            new AkaoInstr(instrSet, 0, 0, 0, origNumInstrs + sampcoll->starting_art_id + i);
        ...
        double freq_multiplier =
            (double)(((ft * 32) + 0x100000) /
                     (double)0x100000);  // this gives us the pitch multiplier value ex. 1.05946
        double cents = log(freq_multiplier) / log((double)2) * 1200;
        if (art->fineTune < 0)
            cents -= 1200;
        rgn->fineTune = (short)cents;
