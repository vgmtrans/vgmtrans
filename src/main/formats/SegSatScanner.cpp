...
void SegSatScanner::Scan(RawFile *file, void *info) {
    ...
    for (uint32_t i = 0; i + 4 < nFileLength; i++) {
        ...
        if (buf[j] == 0x01 && buf[j + 1] == 0 && buf[j + 2] == 0 && buf[j + 3] == 0 &&
            buf[j + 4] == 6 && (buf[j + 5] & 0xF0) == 0 /* && buf[j+6] == 0x30*/) {
            SegSatSeq *newSegSatSeq =
                new SegSatSeq(file, i + 5);  // this, pDoc, pDoc->GetWord(i+24 + k*8)-0x8000000);
