...
bool WDInstrSet::GetInstrPointers() {
    ...
    for (uint32_t i = 0; i < dwNumInstrs; i++) {
        ...
        auto *newWDInstr = new WDInstr(this, dwOffset + GetWord(j + (i * 4)), instrLength, 0, i,
...
bool WDInstr::LoadInstr() {
    ...
    while (k * 0x20 < unLength) {
        ...
        rgn->sampOffset = GetWord(k * 0x20 + 0x4 + dwOffset) &
                          0xFFFFFFF0;  // The & is there because FFX points to 0x----C offsets for
        ...
        rgn->pan = [k, this]() {
            auto newpan = std::clamp((GetByte(k * 0x20 + 0x17 + dwOffset) - 128) / 127.0, 0.0, 1.0);
            if (newpan < 0) {
                return 0.5;
            } else {
                return newpan;
        ...
        rgn->fineTune = static_cast<short>(
            ceil(((finetune_table[rgn->fineTune] - 0x10000) * finetune_coeff) - 50));
    ...
    for (uint32_t k = 0; k < aRgns.size(); k++) {
        // Key Ranges
        if (((WDRgn *)aRgns[k])->bFirstRegion ||
            k == 0)  //&& !instrument[i].region[k].bLastRegion) //used in ffx2 0049 YRP
        ...
        if (aRgns[k]->velHigh == aRgns[k - 1]->velHigh) {
            aRgns[k]->velLow = aRgns[k - 1]->velLow;
            aRgns[k]->velHigh =
                0x7F;  // FFX 0022, aka sight of spira, was giving me problems, hence this
            aRgns[k - 1]->velHigh =
                0x7F;  // hDLSFile.aInstrs.back()->aRgns.back()->usVelHigh = 0x7F;
...
WDRgn::WDRgn(WDInstr *instr, uint32_t offset) : VGMRgn(instr, offset, 0x20) {}
