...
std::string_view SnesRsnModule::name() const {
  return "SnesRsn";
}

bool SnesRsnModule::canScan(const SourceFile&, std::span<const u8> bytes) const {
  return hasRarSignature(bytes);
}

ScanResult SnesRsnModule::scan(const ScanInput& input) const {
  ScanResult result;
...
void registerSnesRsnModule(FormatRegistry& registry) {
  registry.add(std::make_unique<SnesRsnModule>());
}
