...
BytePattern PrismSnesScanner::ptnLoadSeq("\xf6\x00\x23\xc4\x06\xfc\xf6\x00"
...
BytePattern PrismSnesScanner::ptnExecVCmd("\x8d\x00\xf7\x26\x10\x1f\x3a\x26"
...
BytePattern PrismSnesScanner::ptnSetDSPd("\x8d\x16\xe8\x18\xda\x00\x8d\x07"
                                         "\xcd\x7d\x3f\x05\x16",
                                         "x?x?x?xx"
                                         "xxx??",
                                         13);
...
BytePattern PrismSnesScanner::ptnLoadInstr("\xf8\x24\xf7\x26\x3a\x26\xd5\x50"
...
// 14f2: 90 02     bcc   $14f6
// 14f4: ab 2a     inc   $2a
BytePattern PrismSnesScanner::ptnLoadInstrTuning("\xf8\x42\xf5\x00\x21\x60\x84\x2a"
...
void PrismSnesScanner::SearchForPrismSnesFromARAM(RawFile *file) {
    ...
    } else if (file->GetShort(addrVoiceCmdAddressTable) ==
               file->GetShort(addrVoiceCmdAddressTable + 10)) {
    ...
    if (file->SearchBytePattern(ptnSetDSPd, ofsSetDSPd)) {
        uint16_t addrDspRegTabled =
            file->GetByte(ofsSetDSPd + 3) | (file->GetByte(ofsSetDSPd + 1) << 8);
    ...
    PrismSnesInstrSet *newInstrSet =
        new PrismSnesInstrSet(file, version, spcDirAddr, addrADSR1Table, addrADSR2Table,
                              adsrTuningTableHigh, adsrTuningTableLow);
