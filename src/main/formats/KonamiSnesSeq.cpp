...
const uint8_t KonamiSnesSeq::PAN_VOLUME_LEFT_V1[] = {0x00, 0x05, 0x0c, 0x14, 0x1e, 0x28, 0x32,
                                                     0x3c, 0x46, 0x50, 0x59, 0x62, 0x69, 0x6f,
                                                     0x74, 0x78, 0x7b, 0x7d, 0x7e, 0x7e, 0x7f};

const uint8_t KonamiSnesSeq::PAN_VOLUME_RIGHT_V1[] = {0x7f, 0x7e, 0x7e, 0x7d, 0x7b, 0x78, 0x74,
                                                      0x6f, 0x69, 0x62, 0x59, 0x50, 0x46, 0x3c,
                                                      0x32, 0x28, 0x1e, 0x14, 0x0c, 0x05, 0x00};

const uint8_t KonamiSnesSeq::PAN_VOLUME_LEFT_V2[] = {0x00, 0x0a, 0x18, 0x28, 0x3c, 0x50, 0x64,
                                                     0x78, 0x8c, 0xa0, 0xb2, 0xc4, 0xd2, 0xde,
                                                     0xe8, 0xf0, 0xf6, 0xfa, 0xfc, 0xfc, 0xfe};

const uint8_t KonamiSnesSeq::PAN_VOLUME_RIGHT_V2[] = {0xfe, 0xfc, 0xfc, 0xfa, 0xf6, 0xf0, 0xe8,
                                                      0xde, 0xd2, 0xc4, 0xb2, 0xa0, 0x8c, 0x78,
                                                      0x64, 0x50, 0x3c, 0x28, 0x18, 0x0a, 0x00};
...
KonamiSnesSeq::KonamiSnesSeq(RawFile *file, KonamiSnesVersion ver, uint32_t seqdataOffset,
                             std::wstring newName)
...
bool KonamiSnesTrack::ReadEvent(void) {
    ...
    std::map<uint8_t, KonamiSnesSeqEventType>::iterator pEventType =
    ...
    switch (eventType) {
        ...
        case EVENT_GAIN: {
            ...
            desc << L"GAIN: " << (int)newGAINAmount << L" ($" << std::hex << std::setfill(L'0')
                 << std::setw(2) << std::uppercase << (int)newGAIN << L")";
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"GAIN", desc.str().c_str(),
        ...
        case EVENT_PAN: {
            ...
            if (instrumentPanOff) {
                AddGenericEvent(beginOffset, curOffset - beginOffset, L"Per-Instrument Pan Off",
                                desc.str().c_str(), CLR_PAN, ICON_CONTROL);
            } else if (instrumentPanOn) {
                AddGenericEvent(beginOffset, curOffset - beginOffset, L"Per-Instrument Pan On",
                                desc.str().c_str(), CLR_PAN, ICON_CONTROL);
            } else {
                uint8_t volumeLeft;
                uint8_t volumeRight;
        ...
        case EVENT_RANDOM_PITCH: {
            ...
            curOffset += 2;
            desc << L"Rate: " << (int)envRate << L"  Pitch Mask: $" << std::hex
                 << std::setfill(L'0') << std::setw(4) << std::uppercase << (int)envPitchMask;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Random Pitch",
        ...
        case EVENT_LOOP_START_2: {
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Loop Start #2",
                            desc.str().c_str(), CLR_LOOP, ICON_STARTREP);
        ...
        case EVENT_LOOP_END_2: {
            ...
            if (times == 0) {
                ...
                AddGenericEvent(beginOffset, curOffset - beginOffset, L"Loop End #2",
                                desc.str().c_str(), CLR_LOOP, ICON_STARTREP);
            }
        ...
        case EVENT_TEMPO_FADE: {
            ...
            desc << L"BPM: " << parentSeq->GetTempoInBPM(newTempo) << L"  Fade Length: "
                 << (int)fadeSpeed;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Tempo Fade", desc.str().c_str(),
        ...
        case EVENT_ADSR1: {
            ...
            desc << L"ADSR(1): $" << std::hex << std::setfill(L'0') << std::setw(2)
                 << std::uppercase << (int)newADSR1;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR(1)", desc.str().c_str(),
                            CLR_ADSR, ICON_CONTROL);
            break;
        }
        ...
        case EVENT_ADSR2: {
            ...
            desc << L"ADSR(2): $" << std::hex << std::setfill(L'0') << std::setw(2)
                 << std::uppercase << (int)newADSR2;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR(2)", desc.str().c_str(),
                            CLR_ADSR, ICON_CONTROL);
            break;
        }
        ...
        case EVENT_PORTAMENTO: {
            ...
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Portamento", desc.str().c_str(),
        ...
        case EVENT_PITCH_ENVELOPE_V1: {
            ...
            desc << L"Delay: " << (int)pitchEnvDelay << L"  Speed: " << (int)pitchEnvSpeed
                 << L"  Depth: " << (int)-pitchEnvDepth;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pitch Envelope",
                            desc.str().c_str(), CLR_MODULATION, ICON_CONTROL);
            break;
        }
        ...
        case EVENT_PITCH_ENVELOPE_V2: {
            ...
            desc << L"Delay: " << (int)pitchEnvDelay << L"  Length: " << (int)pitchEnvLength
                 << L"  Offset: " << (int)-pitchEnvOffset << L" semitones" << L"  Delta: "
                 << (pitchDelta / 256.0) << L" semitones";
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pitch Envelope",
                            desc.str().c_str(), CLR_MODULATION, ICON_CONTROL);
            break;
        }
        ...
        case EVENT_PITCH_SLIDE_V2: {
            ...
            if (arg2 != 0) {
                ...
                desc << L"Arg4: " << (int)arg4 << L"  Arg5: " << (int)arg5 << L"  Arg6: "
                     << (int)arg6;
            }
        ...
        case EVENT_PITCH_SLIDE_V3: {
            ...
            desc << L"Delay: " << (int)pitchSlideDelay << L"  Length: " << (int)pitchSlideLength
                 << L"  Final Note: " << (int)pitchSlideNoteNumber << L"  Delta: "
                 << (pitchDelta / 256.0) << L" semitones";
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pitch Slide",
                            desc.str().c_str(), CLR_PITCHBEND, ICON_CONTROL);
            break;
        ...
        case EVENT_ECHO: {
            ...
            desc << L"EON: " << (int)echoChannels << L"  EVOL(L): " << (int)echoVolumeL
                 << L"  EVOL(R): " << (int)echoVolumeR;
        ...
        case EVENT_ECHO_PARAM: {
            ...
            desc << L"EDL: " << (int)echoDelay << L"  EFB: " << (int)echoFeedback << L"  Arg3: "
                 << (int)echoArg3;
        ...
        case EVENT_LOOP_WITH_VOLTA_START: {
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Loop With Volta Start",
                            desc.str().c_str(), CLR_LOOP, ICON_STARTREP);
        ...
        case EVENT_LOOP_WITH_VOLTA_END: {
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Loop With Volta End",
                            desc.str().c_str(), CLR_LOOP, ICON_STARTREP);
            ...
            if (voltaEndMeansPlayFromStart) {
                ...
                // third time - start of first volta bracket: play the new bracket (or quit from the
                // entire loop)
        ...
        case EVENT_PAN_FADE: {
            ...
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pan Fade", desc.str().c_str(),
        ...
        case EVENT_ADSR_GAIN: {
            ...
            desc << L"ADSR(1): $" << std::hex << std::setfill(L'0') << std::setw(2)
                 << std::uppercase << (int)newADSR1 << L"  ADSR(2): $" << (int)newADSR2
                 << L"  GAIN: $" << (int)newGAIN;
            AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR(2)", desc.str().c_str(),
                            CLR_ADSR, ICON_CONTROL);
            break;
