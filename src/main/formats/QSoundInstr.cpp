  ...
  if (fmt_version <= VER_115) {
    ...
    for (uint32_t bank = 0; bank < num_instr_banks; bank++)
      for (uint32_t i = 0; i < 256; i++) {
        ...
        wstring name = ss.str();
        aInstrs.push_back(new QSoundInstr(this,
...
QSoundInstr::QSoundInstr(VGMInstrSet *instrSet,
                         uint32_t offset,
                         uint32_t length,
                         uint32_t theBank,
...
QSoundSampColl::QSoundSampColl(RawFile *file,
                               QSoundInstrSet *theinstrset,
