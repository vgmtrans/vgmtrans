...
// author of so2mml. The code is based on his write-up of the format specifications.  Many thanks to
// him.
...
bool TriAcePS1Seq::GetTrackPointers(void) {
    ...
    for (int i = 0; i < 32; i++)
        if (TrkInfos[i].trkOffset != 0) {
            ...
            VGMHeader *TrkInfoBlock =
                TrkInfoHeader->AddHeader(dwOffset + 0x16 + 6 * i, 6, L"Track Info");
        }
    return true;
}
...
bool TriAcePS1Track::ReadEvent(void) {
    ...
    if (status_byte <= 0x7F) {
        ...
        if (!impliedNoteDur)
            ...
            note_dur = impliedNoteDur;
        if (!impliedVelocity)
            velocity = GetByte(curOffset++);
        else
        ...
        switch (status_byte) {
            case 0x80:
                AddGenericEvent(beginOffset, curOffset - beginOffset, L"Score Pattern End", L"",
                                CLR_TRACKEND);
                return false;
            ...
            case 0x8D:
                AddGenericEvent(beginOffset, curOffset - beginOffset, L"Dal Segno: start point",
                                L"", CLR_UNKNOWN);
                break;
            ...
            case 0x8E:
                curOffset++;
                AddGenericEvent(beginOffset, curOffset - beginOffset, L"Dal Segno: end point", L"",
                                CLR_UNKNOWN);
                break;
            ...
            case 0x9E:
                ...
                AddGenericEvent(beginOffset, curOffset - beginOffset, L"Imply Note Params", L"",
                                CLR_CHANGESTATE);
                break;
...
// The following two functions are overridden so that events become children of the Score Patterns
// and not the tracks.
