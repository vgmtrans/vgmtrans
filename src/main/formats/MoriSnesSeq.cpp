...
bool MoriSnesTrack::ReadEvent(void) {
    ...
    std::map<uint8_t, MoriSnesSeqEventType>::iterator pEventType =
    ...
    switch (eventType) {
        ...
        case EVENT_NOTE_WITH_PARAM: {
            ...
            if (prevTiedNoteIter != tiedNoteKeys.end()) {
                ...
                AddNoteByDur(beginOffset, curOffset - beginOffset, key, spcNoteVelocity, dur,
                             eventName);
                if (tied) {
                    // register tied note
        ...
        case EVENT_TEMPO: {
            ...
            AddTempoBPM(beginOffset, curOffset - beginOffset,
                        parentSeq->GetTempoInBPM(newTempo, parentSeq->fastTempo));
            break;
        }
        ...
        case EVENT_ECHO_PARAM: {
            ...
            desc << "Delay: " << (int)delay << "  FIR: " << (int)volume
                 << "  volume: " << (int)feedback << "  FIR: " << (int)filterIndex;
        ...
        case EVENT_PITCHBENDRANGE: {
            ...
            AddPitchBendRange(beginOffset, curOffset - beginOffset,
...
void MoriSnesTrack::ParseInstrumentEvents(uint16_t offset, uint8_t instrNum, bool percussion,
                                          uint8_t percNoteKey) {
    MoriSnesSeq *parentSeq = (MoriSnesSeq *)this->parentSeq;
    ...
    while (bContinue) {
        ...
        switch (eventType) {
            ...
            case EVENT_RET: {
                ...
                curOffset = instrCallStack[instrCallStackPtr - 2] |
                            (instrCallStack[instrCallStackPtr - 1] << 8);
                instrCallStackPtr -= 2;
                break;
            }
            ...
            case EVENT_LOOP_END: {
                ...
                if (--count == 0) {
                    ...
                    curOffset = instrCallStack[instrCallStackPtr - 3] |
                                (instrCallStack[instrCallStackPtr - 2] << 8);
                }
