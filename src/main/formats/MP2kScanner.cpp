...
struct EngineParams {
    EngineParams(u32 data)
        : polyphony((data & 0x000F00) >> 8),
          main_vol((data & 0x00F000) >> 12),
          sampling_rate_index((data & 0x0F0000) >> 16),
          dac_bits(17 - ((data & 0xF00000) >> 20)){};
...
void MP2kScanner::Scan(RawFile *file, void *info) {
    ...
    auto song_tbl =
        gsl::make_span<const u32>(reinterpret_cast<const u32 *>(file->begin() + song_tbl_ptr),
...
static auto MP2KPatternSearch(RawFile *file, const char *offset) {
    ...
    static const auto v2_searcher = std::boyer_moore_horspool_searcher(
        SONGSELECT_PATTERN_V2.begin(), SONGSELECT_PATTERN_V2.end());
