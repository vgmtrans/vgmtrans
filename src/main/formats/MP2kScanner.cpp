...
static bool test_pointer_validity(RawFile *file, off_t offset, uint32_t inGBA_length) {
    ...
    return params.main_vol != 0 && params.polyphony <= 12 && params.dac_bits >= 6 &&
           params.dac_bits <= 9 && params.sampling_rate_index >= 1 &&
           params.sampling_rate_index <= 12 && song_tbl_adr < inGBA_length &&
           file->GetWord(offset + 4) < 256 && ((file->GetWord(offset) & 0xff000000) == 0);
}
...
bool MP2kScanner::Mp2kDetector(RawFile *file, uint32_t &mp2k_offset) {
    ...
    const uint8_t CODESEG_MAIN[M4A_MAIN_PATT_COUNT][M4A_MAIN_LEN] = {{0x00, 0xB5}};
    ...
    while (m4a_selectsong_search_offset != -1) {
        m4a_selectsong_offset = LooseSearch(file, CODESEG_SELECTSONG, sizeof(CODESEG_SELECTSONG),
    ...
    bool valid_m16 =
        test_pointer_validity(file, m4a_main_offset - 16, file->size());  // For most games
    bool valid_m32 =
        test_pointer_validity(file, m4a_main_offset - 32, file->size());  // For pokemon
