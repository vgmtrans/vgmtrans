...
BytePattern HudsonSnesScanner::ptnGetSeqTableAddrV0("\xf6\x6b\x0f\xc4\x0d\xfc\xf6\x6b"
...
// 08f1: c4 19     mov   $19,a
BytePattern HudsonSnesScanner::ptnGetSeqTableAddrV1V2("\xe5\xc2\x07\xec\xc3\x07\xda\x13"
...
BytePattern HudsonSnesScanner::ptnLoadTrackAddress("\xe8\x00\xd4\x88\xd4\x90\xd5\x32"
...
// 0bfa: 8d 5d     mov   y,#$5d
// 0bfc: 4f 0c     pcall $0c
BytePattern HudsonSnesScanner::ptnLoadDIRV0("\xe4\x4b\x8d\x5d\x4f\x0c", "x?xxxx", 6);
...
void HudsonSnesScanner::SearchForHudsonSnesFromARAM(RawFile *file) {
    ...
    if (file->SearchBytePattern(ptnLoadTrackAddress, ofsLoadTrackAddress)) {
        ...
        addrCurrentLoopPoint = file->GetByte(addrCurrentLoopPointPtrLo) |
                               (file->GetByte(addrCurrentLoopPointPtrHi) << 8);
    } else {
        return;
    }
    ...
    if (newSeq->InstrumentTableSize != 0) {
        ...
        HudsonSnesInstrSet *newInstrSet = new HudsonSnesInstrSet(
            file, version, newSeq->InstrumentTableAddress, newSeq->InstrumentTableSize, spcDirAddr,
            addrSampTuningTable, name);
