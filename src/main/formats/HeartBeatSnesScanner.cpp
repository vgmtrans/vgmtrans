...
BytePattern HeartBeatSnesScanner::ptnReadSongList("\xee\xf6\x0d\x0a\xc4\x08\xf6\x19"
...
// 134a: e8 00     mov   a,#$00
BytePattern HeartBeatSnesScanner::ptnSetDIR("\xe8\x02\x8d\x5d\x3f\x48\x14\xe8"
                                            "\x00",
                                            "x?xxx??x"
                                            "x",
                                            9);
...
// 152b: ee        pop   y
BytePattern HeartBeatSnesScanner::ptnLoadSRCN("\x3f\xc3\x12\x8d\x06\xcf\xfd\x6d"
...
// 0be2: c4 3c     mov   $3c,a
BytePattern HeartBeatSnesScanner::ptnSaveSeqHeaderAddress("\xf5\xe0\x08\x28\x0f\xfd\xf6\x0d"
...
void HeartBeatSnesScanner::SearchForHeartBeatSnesFromARAM(RawFile *file) {
    ...
    for (int8_t songIndexCandidate = 0; songIndexCandidate < maxSongIndex; songIndexCandidate++) {
        uint16_t addrSeqHeader = file->GetByte(addrSongListLo + songIndexCandidate) |
                                 (file->GetByte(addrSongListHi + songIndexCandidate) << 8);
        if (addrSeqHeader == 0) {
            break;
        }
    ...
    uint16_t addrSeqHeader = file->GetByte(addrSongListLo + songIndex) |
                             (file->GetByte(addrSongListHi + songIndex) << 8);
    ...
    HeartBeatSnesInstrSet *newInstrSet = new HeartBeatSnesInstrSet(
        file, version, addrInstrTable, instrTableSize, addrSRCNTable, songIndex, spcDirAddr);
