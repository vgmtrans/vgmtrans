		...
		if ((status_byte & 0x1F) != 0)		//if it's not a rest
		{
			...
			if ((noteState&0x40) > 0) // if tie
			{
				...
				else if (key != prevTieNote)
				{
					...
					AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Tie"), L"", CLR_NOTEON);
				bPrevNoteTie = true;
				prevTieNote = key;
			}
			else
			...
			AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Rest"), L"", CLR_REST);
		AddTime(delta);
	}
	else
	{
		...
		case 0x00 :
			noteState ^= 0x20;
			AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Note State xor 0x20 (change duration table)"), L"", CLR_CHANGESTATE);
			break;
		case 0x01 :
			noteState ^= 0x40;
			AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Note State xor 0x40 (Toggle tie)"), L"", CLR_CHANGESTATE);
			break;
		case 0x02 :
			noteState |= (1 << 4);
			AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Note State |= 0x10 (change duration table)"), L"", CLR_CHANGESTATE);
			break;
		case 0x03 :
			noteState ^= 8;
			AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Note State xor 8 (change octave)"), L"", CLR_CHANGESTATE);
			break;
		case 0x04 :
			noteState &= 0x97;
			noteState |= GetByte(curOffset++);
			AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Change Note State (& 0x97)"), L"", CLR_CHANGESTATE);
			break;
		case 0x05 :
			{
		...
		case 0x06 :
			...
			AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Set Duration"), L"", CLR_CHANGESTATE);
			break;
		case 0x07 :
		...
		case 0x09 :					//effectively sets the octave
			noteState &= 0xF8;
			noteState |= GetByte(curOffset++);
			AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Set Octave"), L"", CLR_CHANGESTATE);
			break;
		...
		case 0x0D :	
			{
				...
				//AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Portamento"), L"", CLR_PITCHBEND);
			}
			break;
		case 0x0E :				//loop
			loopNum = 0;
...
theLoop:	if (loop[loopNum] == 0 && loopOffset[loopNum] == 0)						//first time hitting loop
			...
			AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Loop"), L"", CLR_LOOP);
...
loopBreak:	if (loop[loopNum]-1 == 0)
			{
				...
				noteState |= GetByte(curOffset++);
				{
					...
					AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Loop Break"), L"", CLR_LOOP);
					curOffset += jump;
				}
			}
			else
		...
		case 0x1A :	//master(?) vol
			{
				//curOffset++;
				vol = GetByte(curOffset++);
				AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Master Volume"), L"", CLR_UNKNOWN);
		...
		case 0x1F :
			{
				...
				else
				{
					...
					AddGenericEvent(beginOffset, curOffset-beginOffset, _T("Bank Change"), L"", CLR_PROGCHANGE);
				}
		...
		case 0x23 :
			...
			break;
		default :
			AddGenericEvent(beginOffset, curOffset-beginOffset, _T("UNKNOWN"), L"", CLR_UNRECOGNIZED);
		}
	}
	return true;
}
