...
class AkaoSnesRgn : public VGMRgn {
   public:
    ...
    bool InitializeRegion(uint8_t srcn, uint32_t spcDirAddr, uint16_t addrADSRTable);
...
class AkaoSnesDrumKitRgn : public AkaoSnesRgn {
   public:
    ...
    AkaoSnesDrumKitRgn(AkaoSnesDrumKit *instr, AkaoSnesVersion ver, uint16_t addrTuningTable);
    virtual ~AkaoSnesDrumKitRgn(void);

    bool InitializePercussionRegion(uint8_t srcn, uint32_t spcDirAddr, uint16_t addrADSRTable,
                                    uint16_t addrDrumKitTable);
