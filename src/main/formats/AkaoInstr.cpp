...
bool AkaoDrumKit::LoadInstr() {
    uint32_t j = dwOffset;  // j = the end of the last instrument of the instrument table ie, the
                            // beginning of drumkit data
    ...
    for (uint32_t i = 0; (i < 128) && (j < endOffset); i++) {
        ...
        if ((GetWord(j) != 0) && (GetWord(j + 4) != 0)) {
            ...
            AkaoRgn *rgn =
                (AkaoRgn *)AddRgn(new AkaoRgn(this, j, 8, lowKey, highKey, assoc_art_id));
...
bool AkaoRgn::LoadRgn() {
    ...
    // isn't the first region of the instr
    //{
    //	keyLow = keyHigh+1;
    //	Alert(_T("Funkiness going down: lowKey > highKey (also, k > 0)"));
    //}

    /*		if ((k > 0) && (pDoc->GetByte(aInstrs[i]->info_ptr + (k-1)*8 + 1) ==
       aInstrs[i]->aRegions[k]->first_key))
            {
                    if ((k > 1) && (pDoc->GetByte(aInstrs[i]->info_ptr + (k-2)*8 + 1) ==
       aInstrs[i]->aRegions[k]->first_key))
                    {
                            aInstrs[i]->aRegions[k]->first_key += 5;
                            if (aInstrs[i]->info_ptr + (k+1)*8 < aInstrs[i+1]->info_ptr - 8)  //if
       there's another region in the instrument (k+1) aInstrs[i]->aRegions[k]->last_key =
       pDoc->GetByte(aInstrs[i]->info_ptr + (k+1)*8 + 2) - 1; else aInstrs[i]->aRegions[k]->last_key
       = 0x7F; aInstrs[i]->aRegions[k-1]->first_key = aInstrs[i]->aRegions[k]->first_key-5;
                            aInstrs[i]->aRegions[k-1]->last_key = aInstrs[i]->aRegions[k]->first_key
       -1; aInstrs[i]->aRegions[k-2]->first_key = aInstrs[i]->aRegions[k]->first_key-12;
                            aInstrs[i]->aRegions[k-2]->last_key =
       aInstrs[i]->aRegions[k]->first_key-6; if (k == 2)	//if this is the third region of the
       instrument, then we need to make sure the first region's first key is 0
       ...
       pDoc->GetByte(aInstrs[i]->info_ptr + (k+1)*8 + 2) - 1; else aInstrs[i]->aRegions[k]->last_key
       = 0x7F; aInstrs[i]->aRegions[k-1]->first_key = aInstrs[i]->aRegions[k]->first_key-5;
                            aInstrs[i]->aRegions[k-1]->last_key =
       aInstrs[i]->aRegions[k]->first_key-1; if (k == 1)	//if this is the third region of the
       instrument, then we need to make sure the first region's first key is 0
    ...
    // aInstrs[i]->aRegions[k]->sample_offset = GetWord(sampleinfo_offset+
    // aInstrs[i]->aRegions[k]->assoc_art_id *0x10); aInstrs[i]->aRegions[k]->loop_point =
    // GetWord(sampleinfo_offset+ aInstrs[i]->aRegions[k]->assoc_art_id *0x10 + 4) -
    // GetWord(sampleinfo_offset+ instrument[i].region[k].assoc_art_id *0x10 +
    // 0);//GetWord(sampleinfo_offset+ instrument[i].region[k].assoc_art_id *0x10 + 4) -
    // (instrument[i].region[k].sample_offset + sample_section_offset);
    // aInstrs[i]->aRegions[k]->fine_tune =		GetShort(sampleinfo_offset+
    // aInstrs[i]->aRegions[k]->assoc_art_id *0x10 + 8); aInstrs[i]->aRegions[k]->unity_key =
    // GetShort(sampleinfo_offset+ aInstrs[i]->aRegions[k]->assoc_art_id *0x10 + 0xA);
    // aInstrs[i]->aRegions[k]->ADSR1 =			GetShort(sampleinfo_offset+
    // aInstrs[i]->aRegions[k]->assoc_art_id *0x10 + 0xC); aInstrs[i]->aRegions[k]->ADSR2 =
    // GetShort(sampleinfo_offset+ aInstrs[i]->aRegions[k]->assoc_art_id *0x10 + 0xE);

    // instrument[i].region[k].vel_range_high =stuff[(instrument[i].info_ptr + k*0x20 + 0x15)];
    // instrument[i].region[k].pan =			stuff[(instrument[i].info_ptr + k*0x20 +
    // 0x17)];
    //	AkaoRgn* rgn = AkaoRgn();
...
bool AkaoSampColl::GetSampleInfo() {
    ...
    // We do this by searchiing for series of 16 0x00 value bytes.  These indicate the beginning of
    // a sample, and they will never be found at any other point within the adpcm sample data.
    ...
    // check the last 10 bytes to make sure they aren't null, if they are, abbreviate things till
    // there is no 0x10 block of null bytes
    ...
    if (sample_section_offset + sample_section_size > rawfile->size())
        sample_section_size = rawfile->size();  //- sample_section_offset;
    // AddItem("Samples", ICON_TRACK, FileVGMItem.pTreeItem, sample_section_offset, 0, 0,
    // &AllSampsVGMItem); //add the parent "Samples" tree item
