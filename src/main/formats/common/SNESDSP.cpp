...
u32 emulateSDSPGAIN(u8 gain,
  ...
  if (mode < 4) { // direct
    ...
    s16 env_prev = (env >= 0x20) ? env - 0x20 : 0; // guess previous value
  ...
  u32 total_samples = tick * SDSP_COUNTER_RATES[rate];
  ...
  if (sf2_envelope_time_ptr != nullptr) {
    ...
    if (mode < 4) { // direct
      ...
      u32 total_samples_full = (0x800 / 0x20) * SDSP_COUNTER_RATES[rate];
...
void convertSNESADSR(u8 adsr1,
  ...
  if (adsr_enabled) {
    ...
    // decay
    s16 env_sustain_start = env;
    if (sl == 7) {
      // no decay
      decay_time = 0;
    }
    else {
      u8 dr_rate = 0x10 | (dr << 1);
...
SNESSampColl::SNESSampColl(const std::string &format, RawFile *rawfile, u32 offset, u32 maxNumSamps) :
    VGMSampColl(format, rawfile, offset, 0),
    spcDirAddr(offset) {
  setDefaultTargets(maxNumSamps);
}

SNESSampColl::SNESSampColl(const std::string &format, VGMInstrSet *instrset, u32 offset, u32 maxNumSamps) :
...
void SNESSampColl::setDefaultTargets(u32 maxNumSamps) {
...
bool SNESSampColl::parseSampleInfo() {
  ...
  for (std::vector<u8>::iterator itr = this->targetSRCNs.begin(); itr != this->targetSRCNs.end(); ++itr) {
    u8 srcn = (*itr);
    ...
    bool loop;
    u32 length = SNESSamp::getSampleLength(rawFile(), addrSampStart, loop);
...
bool SNESSampColl::isValidSampleDir(const RawFile *file, u32 spcDirEntAddr, bool validateSample) {
  ...
  u16 addrSampStart = file->readShort(spcDirEntAddr);
  u16 addrSampLoop = file->readShort(spcDirEntAddr + 2);
  ...
  if (validateSample) {
    bool loop;
    u32 length = SNESSamp::getSampleLength(file, addrSampStart, loop);
    if (length == 0) {
      return false;
    }

    u16 addrSampEnd = addrSampStart + length;
...
SNESSamp::SNESSamp(VGMSampColl *sampColl, u32 offset, u32 length, u32 dataOffset,
                   u32 dataLen, u32 loopOffset, std::string name)
...
u32 SNESSamp::getSampleLength(const RawFile *file, u32 offset, bool &loop) {
  ...
  while (true) {
    ...
    u8 flag = file->readByte(currOffset);
    currOffset += 9;
...
std::vector<u8> SNESSamp::decodeToNativePcm() {
  ...
  for (u32 k = 0; k + 9 <= dataLength; k += 9)  //for every adpcm chunk
  {
...
// static inline s32 absolute(s32 x) {
//   return ((x < 0) ? -x : x);
// }

static inline s32 sclip15(s32 x) {
  return ((x & 16384) ? (x | ~16383) : (x & 16383));
}

// static inline s32 sclamp8(s32 x) {
//   return ((x > 127) ? 127 : (x < -128) ? -128 : x);
// }

// static inline s32 sclamp15(s32 x) {
//   return ((x > 16383) ? 16383 : (x < -16384) ? -16384 : x);
// }

static inline s32 sclamp16(s32 x) {
  return ((x > 32767) ? 32767 : (x < -32768) ? -32768 : x);
}

void SNESSamp::decompBRRBlk(s16 *pSmp, const BRRBlk *pVBlk, s32 *prev1, s32 *prev2) {
  ...
  for (int i = 0; i < 8; i++) {
    s8 sample1 = pVBlk->brr[i];
    s8 sample2 = sample1 << 4;
    sample1 >>= 4;
    sample2 >>= 4;

    for (int nybble = 0; nybble < 2; nybble++) {
      s32 out = nybble ? sample2 : sample1;
