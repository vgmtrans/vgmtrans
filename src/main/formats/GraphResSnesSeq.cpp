...
GraphResSnesSeq::GraphResSnesSeq(RawFile *file, GraphResSnesVersion ver, uint32_t seqdataOffset,
                                 std::string newName)
    ...
    AlwaysWriteInitialTempo(60000000.0 /
                            ((125 * 0x85) * SEQ_PPQN));  // good ol' frame-based sequence!
...
bool GraphResSnesTrack::ReadEvent(void) {
    ...
    std::map<uint8_t, GraphResSnesSeqEventType>::iterator pEventType =
    ...
    switch (eventType) {
        ...
        case EVENT_NOTE: {
            ...
            uint8_t durRate =
                std::max(durationRate, (uint8_t)8);  // rate > 8 will cause unexpected result
            ...
            if (key == 7) {
                ...
                if (prevNoteSlurred && midiKey == prevNoteKey) {
                    desc << "Duration: " << dur;
                    AddGenericEvent(beginOffset, curOffset - beginOffset, "Tie", desc.str(),
                                    CLR_TIE, ICON_NOTE);
                    MakePrevDurNoteEnd(GetTime() + dur);
                    AddTime(len);
                } else {
                    AddNoteByDur(beginOffset, curOffset - beginOffset, midiKey, vel, dur,
        ...
        case EVENT_INSTANT_OCTAVE: {
            ...
            desc << "Octave: " << octave;
            AddGenericEvent(beginOffset, curOffset - beginOffset, "Octave", desc.str(),
                            CLR_CHANGESTATE);
            break;
        }
        ...
        case EVENT_LOOP_START: {
            AddGenericEvent(beginOffset, curOffset - beginOffset, "", desc.str(), CLR_LOOP,
                            ICON_STARTREP);
        ...
        case EVENT_LOOP_END: {
            ...
            desc << "Times: " << count << "  Destination: $" << std::hex << std::setfill('0')
        ...
        case EVENT_DSP_WRITE: {
            ...
            desc << "Register: $" << std::hex << std::setfill('0') << std::setw(2)
                 << std::uppercase << (int)dspReg << "  Value: $" << (int)dspValue;
            AddGenericEvent(beginOffset, curOffset - beginOffset, "Write to DSP", desc.str(),
                            CLR_CHANGESTATE);
            break;
        }

        case EVENT_NOISE_TOGGLE: {
            AddGenericEvent(beginOffset, curOffset - beginOffset, "Noise On/Off", desc.str(),
        ...
        case EVENT_MASTER_VOLUME_FADE: {
            ...
            AddGenericEvent(beginOffset, curOffset - beginOffset, "Master Volume Fade", desc.str(),
        ...
        case EVENT_RET: {
            AddGenericEvent(beginOffset, curOffset - beginOffset, "End Pattern", desc.str(),
                            CLR_LOOP, ICON_ENDREP);
        ...
        case EVENT_DEFAULT_LENGTH: {
            ...
            AddGenericEvent(beginOffset, curOffset - beginOffset, "Default Note Length",
