...
void AkaoSnesScanner::searchForAkaoSnesFromARAM(RawFile *file) {
  ...
  if (file->searchBytePattern(ptnReadNoteLengthV4, ofsReadNoteLength)) {
    // V4 stores the note length table address at ofsReadNoteLength + 6.
    verReadNoteLength = AKAOSNES_V4;
  }
  else if (file->searchBytePattern(ptnReadNoteLengthV2, ofsReadNoteLength)) {
    // V1/V2 store the note length table address at ofsReadNoteLength + 8.
  ...
  u32 ofsReadSeqHeader;
  u16 addrSeqHeader;
  u16 addrAPURelocBase;
  bool relocatable;
  ...
  const u8 SD2_VCMD_LEN_TABLE[] = {
      0x01, 0x02, 0x01, 0x02, 0x02, 0x03, 0x00, 0x03, 0x00, 0x02, 0x00, 0x01,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x01, 0x01, 0x01,
      0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x01, 0x00, 0x00, 0x01, 0x02, 0x01,
      0x02, 0x02, 0x01, 0x03, 0x02, 0x02, 0x00, 0x01, 0x00, 0x00};
  const u8 RS2_VCMD_LEN_TABLE[] = {
  ...
  const u8 BSGAME_VCMD_LEN_TABLE[] = {
      0x01, 0x02, 0x01, 0x02, 0x02, 0x03, 0x00, 0x03, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00,
      0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01,
      0x02, 0x01, 0x02, 0x01, 0x03, 0x02, 0x01, 0x01, 0x01, 0x03, 0x00, 0x00, 0x01, 0x00, 0x00};
  if (file->matchBytes(FF4_VCMD_LEN_TABLE, addrVCmdLengthTable, sizeof(FF4_VCMD_LEN_TABLE))) {
    ...
    u16 addrVCmdF9 = file->readShort(addrVCmdAddressTable + 53 * 2);
