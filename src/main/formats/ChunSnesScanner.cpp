...
BytePattern ChunSnesScanner::ptnLoadSeqSummerV2("\xd5\x1d\x05\xc9\x66\x05\x2d\xe5"
...
BytePattern ChunSnesScanner::ptnLoadSeqWinterV1V2("\xc9\xf8\x03\xfd\xf6\xc7\x04\x8f"
...
BytePattern ChunSnesScanner::ptnLoadSeqWinterV3("\xc9\x09\x04\xfd\xf6\xd8\x04\x8f"
...
// 0ecd: c9 66 05  mov   $0566,x
// 0ed0: 2d        push  a
BytePattern ChunSnesScanner::ptnSaveSongIndexSummerV2("\x3f\xc3\x0f\xb0\xce\xd5\x1d\x05"
                                                      "\xc9\x66\x05\x2d",
                                                      "x??x?x??"
                                                      "x??x",
                                                      12);
...
BytePattern ChunSnesScanner::ptnDSPInitTable("\x6c\xf0\x7d\x01\x6d\xf7\x1c\x7f"
...
BytePattern ChunSnesScanner::ptnProgChangeVCmdSummer("\x3f\xaa\x15\x2d\xf5\xdd\x03\xfd"
...
BytePattern ChunSnesScanner::ptnProgChangeVCmdWinter("\x3f\xbd\x19\x2d\xf5\xa3\x02\xfd"
...
void ChunSnesScanner::SearchForChunSnesFromARAM(RawFile *file) {
        ...
        if (addrCurrentPos != 0) {
            ...
            for (int8_t songIndexCandidate = 0; songIndexCandidate < 0x7f; songIndexCandidate++) {
                ...
                uint16_t addrSeqHeader =
                    file->GetShort(addrSeqEntry + CHUNSNES_SEQENT_OFFSET_OF_HEADER);
    ...
    if (file->SearchBytePattern(ptnProgChangeVCmdSummer, ofsProgChangeVCmd)) {
        addrInstrSetTable =
            file->GetByte(ofsProgChangeVCmd + 14) | (file->GetByte(ofsProgChangeVCmd + 17) << 8);
        addrSampNumTable = file->GetShort(ofsProgChangeVCmd + 47);
        addrSampleTable =
            file->GetByte(ofsProgChangeVCmd + 67) | (file->GetByte(ofsProgChangeVCmd + 70) << 8);
        ...
        addrSampleTable =
            file->GetByte(ofsProgChangeVCmd + 97) | (file->GetByte(ofsProgChangeVCmd + 100) << 8);
    ...
    ChunSnesInstrSet *newInstrSet = new ChunSnesInstrSet(
        file, version, addrInstrSet, addrSampNumTable, addrSampleTable, spcDirAddr);
