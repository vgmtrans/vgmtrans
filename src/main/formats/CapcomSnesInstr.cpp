...
bool CapcomSnesInstrSet::GetInstrPointers() {
    ...
    SNESSampColl *newSampColl =
        new SNESSampColl(CapcomSnesFormat::name, this->rawfile, spcDirAddr, usedSRCNs);
...
CapcomSnesRgn::CapcomSnesRgn(CapcomSnesInstr *instr, uint32_t offset) : VGMRgn(instr, offset, 6) {
