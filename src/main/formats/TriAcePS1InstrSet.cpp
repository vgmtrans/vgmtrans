...
bool TriAcePS1InstrSet::GetHeaderInfo() {
    VGMHeader *header =
        AddHeader(dwOffset, sizeof(TriAcePS1InstrSet::_InstrHeader));  // 1,Sep.2009 revise
    ...
    sampColl = new PSXSampColl(TriAcePS1Format::name, this, dwOffset + instrSectionSize,
                               unLength - instrSectionSize);
...
bool TriAcePS1InstrSet::GetInstrPointers() {
    uint32_t firstWord =
        GetWord(dwOffset + sizeof(TriAcePS1InstrSet::_InstrHeader));  // 1,Sep.2009 revise
...
TriAcePS1Instr::TriAcePS1Instr(VGMInstrSet *instrSet, uint32_t offset, uint32_t length,
...
bool TriAcePS1Instr::LoadInstr() {
    ...
    this->bank =
        ((instrinfo.progNum + (instrinfo.bankNum << 8)) & 0xFF80) >> 7;  // 1,Sep.2009 revise
    ...
    for (int i = 0; i < instrinfo.numRgns; i++) {
        ...
        VGMRgn *rgn = new VGMRgn(this, dwOffset + sizeof(InstrInfo) + sizeof(RgnInfo) * i,
        ...
        // rgn->loop.loopStatus = (rgninfo->loopOffset != rgninfo->sampOffset) &&
        // (rgninfo->loopOffset != 0);
        ...
        // Attenuation is not using a linear scale. Will need to look at the original code.  For
        // now, it's ignored.
        //  also, be aware the same scale may be employed for vol and expression events (haven't
        //  investigated).
        // long dlsAtten = -(ConvertPercentVolToAttenDB(rgninfo->attenuation/((double)255)) *
        // DLS_DECIBEL_UNIT); rgn->SetAttenuation(dlsAtten);
        aRgns.push_back(rgn);
    }
    return true;
}
