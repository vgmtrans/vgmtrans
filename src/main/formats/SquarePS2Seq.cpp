...
BGMSeq::BGMSeq(RawFile *file, uint32_t offset) : VGMSeq(SquarePS2Format::name, file, offset) {
...
bool BGMTrack::ReadEvent(void) {
    ...
    switch (status_byte) {
        ...
        case 0x22:
            ...
            // I am positive that volume values in this driver do not align with standard MIDI. (see
            // FFXI 213 Ru'Lude Gardens.psf2 for example)
        ...
        case 0x50:  // found in bgm 048 of FFX-2, that's ost song "101 Eternity ~Memory of
                    // Lightwaves~.psf2"
            curOffset += 3;
            AddUnknown(beginOffset, curOffset - beginOffset);
            break;
