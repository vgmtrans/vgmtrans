...
bool PrismSnesInstrSet::GetInstrPointers() {
    ...
    SNESSampColl *newSampColl =
        new SNESSampColl(PrismSnesFormat::name, this->rawfile, spcDirAddr, usedSRCNs);
...
bool PrismSnesInstr::LoadInstr() {
    ...
    PrismSnesRgn *rgn = new PrismSnesRgn(this, version, srcn, spcDirAddr, addrADSR1Entry,
                                         addrADSR2Entry, addrTuningEntryHigh, addrTuningEntryLow);
...
PrismSnesRgn::PrismSnesRgn(PrismSnesInstr *instr, PrismSnesVersion ver, uint8_t srcn,
                           uint32_t spcDirAddr, uint16_t addrADSR1Entry, uint16_t addrADSR2Entry,
                           uint16_t addrTuningEntryHigh, uint16_t addrTuningEntryLow)
    : VGMRgn(instr, addrADSR1Entry, 0), version(ver) {
    ...
    // put a random release time, it would be better than plain key off (actual music engine never
    // do key off)
