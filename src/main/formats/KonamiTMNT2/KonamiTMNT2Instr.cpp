...
void KonamiTMNT2SampleInstrSet::addInstrInfoChildren(VGMItem* instrInfoItem, uint32_t off) {
...
bool KonamiTMNT2SampleInstrSet::parseMelodicInstrs() {
  ...
  for (auto& instrInfo : m_instrInfos) {
    uint32_t offset = readShort(m_instrTableAddr + instrNum * 2);
...
bool KonamiTMNT2SampleInstrSet::parseDrums() {
  ...
  uint16_t minDrumOffset = -1;
  uint16_t maxDrumOffset = 0;
  for (int i = 0; i < m_drumTables.size(); ++i) {
    uint16_t ptrOffset = m_drumTableAddr + i * 2;
  ...
  maxDrumOffset = 0;
  int drumNum = 0;
  for (uint32_t i = 0; i < m_drumTables.size(); ++i) {
    uint16_t drumBankTablePtr = readShort(m_drumTableAddr + i * 2);
    ...
    for (int j = 0; j < drumBankTable.size(); ++j) {
      uint32_t ptrOffset = drumBankTablePtr + j * 2;
      uint16_t ptr = readShort(ptrOffset);
      minDrumOffset = std::min(minDrumOffset, ptr);
      maxDrumOffset = std::max(maxDrumOffset, ptr);
      drumBankItem->addChild(ptrOffset, 2, "Drum Pointer");

      uint8_t flagsByte = readByte(ptr + 3);
...
bool KonamiTMNT2SampColl::parseSampleInfo() {
  int sampNum = 0;
  std::set<std::tuple<uint32_t, uint32_t, bool>> seenSamples;

  for (auto sampInfo : m_sampInfos) {
    uint32_t sampleOffset = sampInfo.offset;
    uint32_t sampleSize = sampInfo.length;
