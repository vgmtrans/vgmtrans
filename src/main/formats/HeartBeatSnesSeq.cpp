...
bool HeartBeatSnesTrack::ReadEvent(void)
{
	...
	case EVENT_NOTE_LENGTH:
	{
		...
		if (noteParam < 0x80) {
			...
			desc << L"  Duration: " << (int)durIndex << L" (" << (int)spcNoteDurRate << L")" << L"  Velocity: " << (int)velIndex << L" (" << (int)spcNoteVolume << L")";
		}

		AddGenericEvent(beginOffset, curOffset - beginOffset, L"Note Length & Param", desc.str().c_str(), CLR_CHANGESTATE);
		break;
	}
	...
	case EVENT_SLUR_ON:
	{
		AddGenericEvent(beginOffset, curOffset - beginOffset, L"Slur On", desc.str().c_str(), CLR_PORTAMENTO, ICON_CONTROL);
		slur = true;
		break;
	}
	...
	case EVENT_SLUR_OFF:
	{
		AddGenericEvent(beginOffset, curOffset - beginOffset, L"Slur Off", desc.str().c_str(), CLR_PORTAMENTO, ICON_CONTROL);
		slur = false;
		break;
	}
	...
	case EVENT_SUBEVENT:
	{
		...
		std::map<uint8_t, HeartBeatSnesSeqSubEventType>::iterator pSubEventType = parentSeq->SubEventMap.find(subStatusByte);
