    ...
    switch (eventType) {
        ...
        case EVENT_NOTE_LENGTH: {
            ...
            if (noteParam < 0x80) {
                ...
                desc << "  Duration: " << (int)durIndex << " (" << (int)spcNoteDurRate << ")"
                     << "  Velocity: " << (int)velIndex << " (" << (int)spcNoteVolume << ")";
            }

            AddGenericEvent(beginOffset, curOffset - beginOffset, "Note Length & Param",
        ...
        case EVENT_SUBEVENT: {
            ...
            switch (subEventType) {
                ...
                case SUBEVENT_LOOP_AGAIN: {
                    ...
                    desc << "Destination: $" << std::hex << std::setfill('0') << std::setw(4)
                         << std::uppercase << (int)dest;
                    AddGenericEvent(beginOffset, curOffset - beginOffset, "Loop Again",
                                    desc.str().c_str(), CLR_LOOP, ICON_ENDREP);
                ...
                case SUBEVENT_SURROUND: {
                    ...
                    desc << "Invert Left: " << (invertLeft ? "On" : "Off") << "  Invert Right: "
                         << (invertRight ? "On" : "Off");
                    AddGenericEvent(beginOffset, curOffset - beginOffset, "Surround",
