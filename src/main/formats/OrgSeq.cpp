...
OrgSeq::OrgSeq(RawFile *file, uint32_t offset) : VGMSeq(OrgFormat::name, file, offset) {}
...
bool OrgSeq::GetHeaderInfo(void) {
    ...
    for (int i = 0; i < 16; i++) {
        if (GetShort(dwOffset + 0x16 + i * 6))  // if there are notes in this track
        {
            ...
            OrgTrack *newOrgTrack = new OrgTrack(this, dwOffset + 0x12 + 16 * 6 + notesSoFar * 8,
                                                 GetShort(0x16 + i * 6) * 8, i);
