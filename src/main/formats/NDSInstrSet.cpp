    ...
    switch (instrType) {
        ...
        case 0x10: {
            ...
            for (uint8_t i = 0; i < nRgns; i++) {
                VGMRgn *rgn = AddRgn(dwOffset + 2 + i * 12, 12, GetShort(dwOffset + 2 + 2 + i * 12),
                                     lowKey + i, lowKey + i);
...
uint16_t NDSInstr::GetFallingRate(uint8_t DecayTime) {
    ...
    if (DecayTime == 0x7F)
        ...
        realDecay /= DecayTime;  // there is a whole subroutine that seems to resolve simply to
                                 // this.  I have tested all cases
        realDecay &= 0xFFFF;
    }
...
bool NDSWaveArch::GetSampleInfo() {
    ...
    for (uint32_t i = 0; i < nSamples; i++) {
        ...
        uint32_t nonLoopLength =
            GetShort(pSample + 8) * 4;  // if IMA-ADPCM, subtract one for the ADPCM header
...
void NDSSamp::ConvertToStdWave(uint8_t *buf) {
    if (waveType == IMA_ADPCM)
        ...
        for (unsigned int i = 0; i < dataLength;
            ...
            buf[i] ^=
                0x80;  // For whatever reason, the WAV standard has PCM8 unsigned and PCM16 signed
    } else
        GetBytes(dataOff, dataLength, buf);
}
