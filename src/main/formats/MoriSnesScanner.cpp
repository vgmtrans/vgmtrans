...
// 0c50: d0 14     bne   $0c66
// 0c52: 6f        ret
BytePattern MoriSnesScanner::ptnLoadSeq("\x1c\xfd\xf6\xfe\x11\xc4\x04\xf6"
...
BytePattern MoriSnesScanner::ptnSetDIR("\x8f\x11\xf3\x8f\x6c\xf2", "x?xxxx", 6);
...
void MoriSnesScanner::SearchForMoriSnesFromARAM(RawFile *file) {
    ...
    if (addrSongHeaderPtr + 2 <= 0x10000) {
        ...
        if (spcDirAddr != 0) {
            MoriSnesInstrSet *newInstrSet = new MoriSnesInstrSet(
                file, version, spcDirAddr, newSeq->InstrumentAddresses, newSeq->InstrumentHints);
