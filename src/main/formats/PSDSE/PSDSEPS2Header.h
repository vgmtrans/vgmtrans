...
inline bool isV25(uint16_t version) {
  return version == 0x0250;
}

inline uint16_t midiBankNumber(uint16_t bankId) {
  // [Xenosaga Episode I: Der Wille zur Macht]: SsdSeqBankMSB and SsdSeqBankLSB store the two MIDI bank-selector
  // operands in separate bytes of the runtime ID. Pack those seven-bit values contiguously for MIDI, DLS, and SF2.
  return static_cast<uint16_t>(((bankId >> 8) & 0x7f) << 7 | (bankId & 0x7f));
}
...
struct SequenceHeader {
  ...
  bool read(const RawFile* file, uint32_t readOffset) {
    ...
    if (isV2(commonHeaderVersion) || isV25(commonHeaderVersion)) {
      ...
      if (isV25(version)) {
        // [Tokimeki Memorial: Girl's Side]: The unstripped SSD Ver 2.5.0.020304 driver reads the sequence ID from
        // +0x0e, the default wave-bank ID from +0x1c, the track count from +0x1e, and the MIDI channel count from
        // +0x27. SsdSetDataAddress retains the version 2.0 record layout beginning at +0x28.
        fileId = file->readShort(readOffset + 0x0e);
        defaultBankId = file->readShort(readOffset + 0x1c);
        trackCount = file->readByte(readOffset + 0x1e);
        channelCount = file->readByte(readOffset + 0x27);
      } else {
...
struct EffectSetHeader {
  ...
  bool read(const RawFile* file, uint32_t readOffset) {
    ...
    if (isV2(commonHeaderVersion) || isV25(commonHeaderVersion)) {
      ...
      // [Xenosaga Episode I: Der Wille zur Macht]: Version 0x0250 retains these fields, the +0x34 pointer table, and
      // the eight-byte effect record header.
      version = commonHeaderVersion;
...
struct BankHeader {
  ...
  bool read(const RawFile* file, uint32_t readOffset) {
    ...
    if (isV25(commonHeaderVersion)) {
      // [Tokimeki Memorial: Girl's Side]: The unstripped SSD Ver 2.5.0.020304 SsdAddWaveData routine reads the file
      // ID, wave-bank ID, wave and program counts, ADPCM size, and ADPCM offset at the offsets below. It walks
      // 16-bit type and length records from +0x40; type 3 contains 32-byte wave descriptors and type 4 contains a
      // 16-bit program pointer table followed by variable-length programs.
      // [Xenosaga Episode I: Der Wille zur Macht]: SsdAddWaveData registers banks by +0x12 independently of the
      // common-header file ID at +0x0e, so matching values in shipped data are not enforced as a format invariant.
      version = commonHeaderVersion;
      fileId = file->readShort(readOffset + 0x0e);
      bankId = file->readShort(readOffset + 0x12);
      waveCount = file->readByte(readOffset + 0x14);
      programCount = file->readByte(readOffset + 0x15);
      sampleDataSize = file->readWord(readOffset + 0x24);
      sampleDataOffset = file->readWord(readOffset + 0x28);
      if (fileLength < 0x40 || (sampleDataSize != 0 && (sampleDataOffset < 0x40 || sampleDataOffset > fileLength ||
                                                        sampleDataSize > fileLength - sampleDataOffset))) {
        return false;
      }
      internalName = objectName("SWDM", bankId);
      ...
      bool foundTerminator = false;
      uint32_t cursor = readOffset + 0x40;
      const uint32_t fileEnd = readOffset + fileLength;
      while (cursor <= fileEnd && fileEnd - cursor >= 4) {
        const uint16_t type = file->readShort(cursor);
        const uint16_t recordSize = file->readShort(cursor + 2);
        ...
        if (type == 3) {
          ...
          const uint32_t pointerBytes = static_cast<uint32_t>(programCount) * 2;
          if (programDataOffset != 0 || recordSize < 0x10 || static_cast<uint32_t>(recordSize - 0x10) < pointerBytes) {
            return false;
          }
          programDataOffset = cursor + 0x10;
          programDataSize = recordSize - 0x10;
        }
        cursor += recordSize;
      }
    ...
    // dispatches on +0x36. Types 0, 2, 3, and 4 select standard, fixed-address, dynamic-address, and segmented
    // allocation respectively.
    // [Daito Giken Koushiki Pachi-Slot Simulator: 24 - Twenty Four]: The named version 0x0301 routine also
    // dispatches on +0x36, but types 0 and 3 select fixed-address allocation, types 1 and 2 select the two standard
    // allocators, type 4 selects dynamic-address allocation, and type 5 selects segmented allocation.
    // [Tokimeki Memorial: Girl's Side 2nd Kiss]: Version 0x0320 moves the dispatch byte to +0x34 and accepts types 0
    // through 5. Every audited version 0x0320 bank selects type 3.
    waveDataLoaderType = file->readByte(readOffset + (version == 0x0320 ? 0x34 : 0x36));
    const uint8_t maximumLoaderType = version == 0x0300 ? 4 : 5;
    if (waveDataLoaderType > maximumLoaderType) {
      return false;
    }
