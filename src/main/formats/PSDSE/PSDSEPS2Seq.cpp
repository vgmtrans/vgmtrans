...
uint8_t operandCount(uint8_t status, uint16_t version) {
  ...
  if (status == 0xf8 && (PSDSEPS2::isV25(version) || version == 0x0300)) {
    return 3;
  }
  if (version == 0x0320) {
    switch (status) {
...
std::string eventName(uint8_t status, uint16_t version) {
  switch (status) {
    ...
    case 0xa1:
      return "Position";
    case 0xa4:
      return PSDSEPS2::isV2(version) || PSDSEPS2::isV25(version) ? "Dummy 0xA4" : "Cue Point";
    case 0xa6:
      return PSDSEPS2::isV2(version) ? "Dummy 0xA6" : "Master Fader";
    case 0xa7:
    ...
    case 0xad:
      return PSDSEPS2::isV2(version) || PSDSEPS2::isV25(version) || version == 0x0300 ? "Dummy 0xAD" : "Voice Mode";
    case 0xae:
    ...
    case 0xbc:
      return "Gate Time";
    case 0xbd:
      return PSDSEPS2::isV2(version) || PSDSEPS2::isV25(version) || version == 0x0300 ? "Dummy 0xBD" : "Step Relative";
    case 0xbe:
      return PSDSEPS2::isV2(version) || PSDSEPS2::isV25(version) || version == 0x0300 ? "Dummy 0xBE" : "Modulation";
    case 0xbf:
...
bool PSDSEPS2Seq::parseHeader() {
  if (m_header.isEffect) {
    ...
    if (m_header.version == 0x0320) {
      ...
      // [Xenosaga Episode I: Der Wille zur Macht]: Version 0x0250 retains the same effect record fields.
    ...
    if (m_header.version == 0x0320) {
      ...
      // records store zero there, and +6 stores the record extent.
      // [Bakusou Dekotora Densetsu: Otoko Hanamichi Yume Roman]: All 229 audited version 0x0300 effect records store
      // zero at +4 through +5, while +6 stores the record extent.
  ...
  if (PSDSEPS2::isV25(m_header.version)) {
    auto* header = addHeader(offset(), 0x28, "PS2 SMDM v2.5 Header");
    header->addChild(offset(), 4, "Magic");
    header->addChild(offset() + 0x04, 4, "Checksum Key");
    header->addChild(offset() + 0x08, 4, "File Length");
    header->addChild(offset() + 0x0c, 2, "Version");
    ...
    header->addChild(offset() + 0x10, 8, "Creation Timestamp");
    header->addChild(offset() + 0x18, 1, "Initial Master Volume");
    header->addChild(offset() + 0x19, 1, "Reverb Core Flags");
    // [Tokimeki Memorial: Girl's Side]: SsdAddSequenceData and SsdInitSequence leave +0x1a through +0x1b unread,
    // and all 763 audited sequences store zeroes there.
    header->addChild(offset() + 0x1a, 2, "Zero Padding");
    header->addChild(offset() + 0x1c, 2, "Default Wave Bank ID");
    header->addChild(offset() + 0x1e, 1, "Track Count");
    header->addChild(offset() + 0x1f, 1, "Device Priority");
    header->addChild(offset() + 0x20, 1, "Authoring Timing Metadata (Driver Unused)");
    header->addChild(offset() + 0x21, 1, "First Voice Number");
    header->addChild(offset() + 0x22, 1, "Highest Voice Number");
    // [Tokimeki Memorial: Girl's Side]: SsdSetSeqReverb passes +0x23 through +0x26 to the named
    // SsdSetReverbParameter routine as mode, depth, delay, and feedback.
    header->addChild(offset() + 0x23, 1, "Reverb Mode");
    header->addChild(offset() + 0x24, 1, "Reverb Depth");
    header->addChild(offset() + 0x25, 1, "Reverb Delay");
    header->addChild(offset() + 0x26, 1, "Reverb Feedback");
    header->addChild(offset() + 0x27, 1, "Channel Count");
    addBankReference(m_header.defaultBankId);
    return true;
  }
  ...
  // but never consumed, while +0x3a through +0x3b, +0x42 through +0x43, and +0x46 through +0x47 are not read.
  // [Rakugaki Oukoku 2: Maou-jou no Tatakai]: All 14 audited version 0x0301 sequences store zero at +0x3a through
  // +0x3b and +0x46 through +0x47, while +0x42 through +0x43 contains nonzero authoring values.
  // [Tokimeki Memorial: Girl's Side 2nd Kiss]: All 922 audited version 0x0320 sequences confirm the same distinction.
  ...
  header->addChild(offset() + 0x42, 2, "Authoring Metadata (Driver Unused)");
  header->addChild(offset() + 0x44, 1, "First Voice Number");
  header->addChild(offset() + 0x45, 1, "Highest Voice Number");
  header->addChild(offset() + 0x46, 2, "Zero Padding");
...
bool PSDSEPS2Seq::parseTrackPointers() {
  for (const auto& record : m_header.tracks) {
    ...
    if (PSDSEPS2::isV2(m_header.version) || PSDSEPS2::isV25(m_header.version)) {
      ...
      trackHeader->addChild(record.recordOffset + 4, 1,
                            PSDSEPS2::isV25(m_header.version) ? "Track Flags" : "Voice Count");
...
void PSDSEPS2Track::resetVars() {
  ...
  m_repeatOffset = 0;
  m_repeatOctave = m_octave;
  m_dseLoopStack.clear();
  vol = 127;
  expression = 127;
  prevPan = 64;
}
...
void PSDSEPS2Track::addDseBankSelect(uint32_t eventOffset, uint32_t eventLength, const std::string& name) {
  const bool isNewOffset = onEvent(eventOffset, eventLength);
  recordSeqEvent<BankSelectSeqEvent>(isNewOffset, getTime(), m_bankId, eventOffset, eventLength, name);
  addBankSelectNoItem(PSDSEPS2::midiBankNumber(m_bankId));
}
...
bool PSDSEPS2Track::readEvent() {
  ...
  if (m_initialBankPending) {
    ...
    addBankSelectNoItem(PSDSEPS2::midiBankNumber(m_defaultBankId));
  ...
  if (status < 0x80) {
    ...
    // [Daito Giken Koushiki Pachi-Slot Simulator: 24 - Twenty Four]: SsdNormalTrackSequence scales the emitted note
    // duration by the persistent SsdSeqGate value divided by 127; sequence timing remains controlled by delta events.
  ...
  switch (status) {
    ...
    case 0x90:
      if (m_repeatOffset != 0) {
        ...
        // [Daito Giken Koushiki Pachi-Slot Simulator: 24 - Twenty Four]: SsdSeqStop restores the octave saved by
        // SsdSeqRepeat and clears both compact-duration fields before returning to the repeat point.
        m_octave = m_repeatOctave;
        m_afterDelta = 0;
        m_gateDelta = 0;
        m_lastNoteDuration = 0;
    ...
    case 0x97:
      ...
      break;
    case 0x98: {
      const uint8_t rawCount = readByte(curOffset++);
      const uint16_t playCount = rawCount == 0 ? 256 : rawCount;
      // [Daito Giken Koushiki Pachi-Slot Simulator: 24 - Twenty Four]: Four 12-byte loop frames occupy track
      // +0x80 through +0xaf. A fifth nested loop would overwrite the next track's runtime state.
      if (m_dseLoopStack.size() >= 4) {
        addUnknown(beginOffset, curOffset - beginOffset, "Loop Top", "Loop nesting exceeds four frames");
        return false;
      }
      // [Shadow Hearts]: SsdSeqLoopTop stores the encoded count minus one, so zero represents 256 total passes.
      // [Tokimeki Memorial: Girl's Side]: The later version 0x0250 driver retains the same counter and octave-save
      // behavior, confirming that it is not specific to the version 2 track layout.
      // [Tokimeki Memorial: Girl's Side 2nd Kiss]: The stripped version 0x0320 handlers at 0x672c, 0x6774, and
      // 0x67e0 retain these operations with the loop-frame array moved from track +0x80 to +0x90.
      m_dseLoopStack.push_back({curOffset, static_cast<uint16_t>(playCount - 1), 0, m_octave, m_octave});
      loopStack.push_back({curOffset, static_cast<uint32_t>(playCount - 1)});
      addGenericEvent(beginOffset, curOffset - beginOffset, eventName(status, m_version),
                      fmt::format("Play count: {}", playCount), VGMItem::Type::RepeatStart);
      break;
    }
    case 0x99: {
      if (m_dseLoopStack.empty()) {
        addGenericEvent(beginOffset, 1, eventName(status, m_version), "Loop stack is empty",
                        VGMItem::Type::Unknown);
        return false;
      }

      auto& frame = m_dseLoopStack.back();
      addGenericEvent(beginOffset, 1, eventName(status, m_version),
                      fmt::format("Passes remaining after this pass: {}", frame.remainingCount),
                      VGMItem::Type::RepeatEnd);
      if (frame.remainingCount == 0) {
        m_dseLoopStack.pop_back();
        loopStack.pop_back();
        break;
      }

      // [Daito Giken Koushiki Pachi-Slot Simulator: 24 - Twenty Four]: SsdSeqLoopEnd records the address after
      // itself and the current octave for Loop Escape, then restores the Loop Top octave before repeating.
      frame.escapeOffset = curOffset;
      frame.exitOctave = m_octave;
      --frame.remainingCount;
      loopStack.back().remainingCount = frame.remainingCount;
      m_octave = frame.startOctave;
      curOffset = frame.startOffset;
      break;
    }
    case 0x9a: {
      if (m_dseLoopStack.empty()) {
      ...
      auto& frame = m_dseLoopStack.back();
      if (frame.remainingCount == 0 && frame.escapeOffset == 0) {
        addUnknown(beginOffset, 1, eventName(status, m_version), "No preceding Loop End established the destination");
        return false;
      }
      addGenericEvent(beginOffset, 1, eventName(status, m_version), "Skips to after Loop End on the final pass",
                      VGMItem::Type::LoopBreak);
      if (frame.remainingCount == 0) {
        // [Bakusou Dekotora Densetsu: Otoko Hanamichi Yume Roman]: SsdSeqLoopEscape branches only when the saved
        // counter reaches zero, restores the octave captured by Loop End, and removes the active loop frame.
        curOffset = frame.escapeOffset;
        m_octave = frame.exitOctave;
        m_dseLoopStack.pop_back();
        loopStack.pop_back();
      }
      break;
    ...
    case 0xa8: {
      if (m_version == 0x0320) {
        ...
        const uint8_t value = readByte(curOffset++);
        // [Tokimeki Memorial: Girl's Side]: Despite the exported SsdSeqBankMSB name, the version 0x0250 handler
        // preserves the high byte and writes this operand into the low byte. Shipped A8 01, A9 00 pairs select bank
        // 1, matching the adjacent SWDM file ID.
        m_bankId = (PSDSEPS2::isV2(m_version) || PSDSEPS2::isV25(m_version))
                       ? static_cast<uint16_t>((m_bankId & 0xff00) | value)
                       : static_cast<uint16_t>((m_bankId & 0x00ff) | (value << 8));
      }
      addDseBankSelect(beginOffset, curOffset - beginOffset, eventName(status, m_version));
      break;
    }
    case 0xa9: {
      const uint8_t value = readByte(curOffset++);
      m_bankId = (m_version == 0x0320 || PSDSEPS2::isV2(m_version) || PSDSEPS2::isV25(m_version))
                     ? static_cast<uint16_t>((m_bankId & 0x00ff) | (value << 8))
                     : static_cast<uint16_t>((m_bankId & 0xff00) | value);
      addDseBankSelect(beginOffset, curOffset - beginOffset, eventName(status, m_version));
      break;
    ...
    case 0xac: {
      ...
      // [Tokimeki Memorial: Girl's Side]: Bank MSB and Bank LSB commonly occur as adjacent writes, so only the bank
      // state consumed by Program Change is a collection dependency. Recording each partial write creates false
      // references such as 0x0014 before the following command completes 0x5a14.
      if (readMode == READMODE_ADD_TO_UI) {
        parentSeq->addBankReference(m_bankId);
      }
    ...
    case 0xbc: {
      ...
      const bool isEffect = static_cast<PSDSEPS2Seq*>(parentSeq)->m_header.isEffect;
      const std::string detail = isEffect
                                     ? fmt::format("Effect note duration scale: {}/16; 15 shortens by two ticks",
                                                   m_gateTime)
                                     : fmt::format("Note duration scale: {}/127", m_gateTime);
      addGenericEvent(beginOffset, curOffset - beginOffset, eventName(status, m_version),
