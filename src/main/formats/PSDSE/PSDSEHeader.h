...
struct SWDLHeader {
  ...
  bool read(const RawFile* file, uint32_t readOffset) {
    ...
    while (currentOffset < fileEnd) {
      ...
      if (chunkSize > fileEnd - currentOffset - 0x10) {
        return false;
      }
      ...
      // [Disaster: Day of Crisis]: Empty WAVI, PRGI, and PCMD chunks still occupy their 16-byte headers; only EOD
      // terminates the bank. EXT03_00_CF_FS.SWD places a nonempty KGRP after its empty WAVI and PRGI chunks.
