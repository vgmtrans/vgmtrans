...
void PSDSEScanner::scan(RawFile* file, void* offset) {
  ...
  while (scanOffset + 4 <= nFileLength) {
    ...
    if (magic.kind == PSDSE::FileKind::Bank) {
      ...
      if (header.read(file, i)) {
        ...
        if (header.waviOffset != 0 && header.nbwavislots != 0 && !header.hasExternalPcmd) {
          sampColl =
        ...
        if (header.prgiOffset != 0 && header.nbprgislots != 0) {
  ...
  for (PSDSEPS2Seq* seq : ps2Sequences) {
    ...
    for (uint16_t bankId : seq->referencedBanks()) {
      ...
      std::ranges::copy_if(
          g_loadedPS2InstrSets, std::back_inserter(candidates), [seq, bankId](const PSDSEPS2InstrSet* instrSet) {
            return instrSet->m_header.bankId == bankId && instrSet->m_header.version == seq->m_header.version;
          });
      ...
      if (PSDSEPS2::isV2(seq->m_header.version) && candidates.size() > 1) {
      ...
      } else if (PSDSEPS2::isV25(seq->m_header.version) && candidates.size() > 1) {
        PSDSEPS2InstrSet* nearest = nullptr;
        uint64_t nearestDistance = std::numeric_limits<uint64_t>::max();
        const uint64_t sequenceEnd = static_cast<uint64_t>(seq->offset()) + seq->length();
        for (PSDSEPS2InstrSet* instrSet : candidates) {
          const uint64_t bankEnd = static_cast<uint64_t>(instrSet->offset()) + instrSet->length();
          const uint64_t distance = instrSet->offset() >= sequenceEnd
                                        ? instrSet->offset() - sequenceEnd
                                        : (bankEnd <= seq->offset() ? seq->offset() - bankEnd : uint64_t{0});
          if (distance < nearestDistance) {
            nearest = instrSet;
            nearestDistance = distance;
          }
        ...
        if (nearest) {
          // [Tokimeki Memorial: Girl's Side]: SOUND.PAK places a new SWDM immediately after the first SMDM that uses
          // it, while later SMDM objects reuse that bank from the preceding data. Numeric bank IDs can repeat, so
          // physical proximity selects the loaded bank instance.
          candidates = {nearest};
        }
      }
      if (candidates.size() > 1) {
    ...
    for (uint16_t bankId : seq->referencedBanks()) {
      ...
      std::ranges::copy_if(
          g_loadedPS2SampColls, std::back_inserter(candidates), [seq, bankId](const PSDSEPS2SampColl* sampColl) {
            return sampColl->m_header.bankId == bankId && sampColl->m_header.version == seq->m_header.version;
          });
      ...
      if (PSDSEPS2::isV2(seq->m_header.version) && candidates.size() > 1) {
        ...
        PSDSEPS2SampColl* nearest = nullptr;
        uint64_t nearestDistance = std::numeric_limits<uint64_t>::max();
        const uint64_t sequenceEnd = static_cast<uint64_t>(seq->offset()) + seq->length();
        for (PSDSEPS2SampColl* sampColl : candidates) {
          const uint64_t bankEnd = static_cast<uint64_t>(sampColl->offset()) + sampColl->length();
          const uint64_t distance = sampColl->offset() >= sequenceEnd
                                        ? sampColl->offset() - sequenceEnd
                                        : (bankEnd <= seq->offset() ? seq->offset() - bankEnd : uint64_t{0});
          if (distance < nearestDistance) {
            nearest = sampColl;
            nearestDistance = distance;
