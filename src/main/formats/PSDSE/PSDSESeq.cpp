...
bool PSDSESeq::parseHeader() {
  ...
  if (version != 0x0402 && version != 0x0415) {
    return false;
  }
  header->addChild(curOffset + 0x0C, 2, "Version");
  // [Pokemon Mystery Dungeon: Explorers of Sky]: DseFile_CheckHeader returns +0x0e as the sequence file ID.
  // The song selector in seqinfo is independent of this file ID and of the bank selected by sequence events.
  ...
  if (readWordBE(curOffset) == 0x736F6E67) {  // "song"
    if (fileLength < 0x50 + seqInfoSize) {
      return false;
    }
...
bool PSDSESeq::parseTrackPointers() {
  ...
  while (curOffset <= eof && eof - curOffset >= 4) {
    ...
    if (chunkType == 0x74726B20) {  // "trk "
      ...
      if (chunkLen < 4 || chunkLen > eof - (curOffset + 0x10)) {
        return false;
      }
...
void PSDSETrack::resetVars() {
  ...
  // [Pokemon Mystery Dungeon: Explorers of Sky]: ARM9 0x0206e27c passes bank zero to the channel reset.
  // [Line Attack Heroes]: SsdSetupSequencePtr also resets channels with bank zero, independently of song ID.
  currentBankId = 0;
  currentProgram = 0;
...
bool PSDSETrack::readEvent() {
  ...
  if (initialBankPending) {
    // [Line Attack Heroes]: Channel reset establishes bank zero before the first event. SsdSeqBank commands
    // replace this 16-bit SWDL file ID; seqinfo's song selector does not participate in bank selection.
  ...
  if (status_byte <= 0x7F) {
    ...
    // [Pokemon Mystery Dungeon: Explorers of Sky]: ParseDseEvent scales each note's duration by the track's gate
    // value from opcode 0xBC, whose default of 127 leaves the duration unchanged. The ARM routine computes:
    ...
    tieNextNote = false;
    // [Pokemon Mystery Dungeon: Explorers of Sky]: Wait events advance sequence time after the note event, so the
    // note duration must not advance the track independently.
  ...
  switch (status_byte) {
    ...
    case 0x95: {
      // [Line Attack Heroes]: RVL_Default.MAP names this handler SsdSeqDeltaEnvOff. The driver waits until every
      // envelope is off and uses the operand as the polling interval; the interval has no MIDI equivalent.
      curOffset++;
      addGenericEvent(beginOffset, curOffset - beginOffset, "Wait Until Envelope Off", "", VGMItem::Type::Rest);
    } break;
    case 0x98:
      if (repeatOffset != 0) {
        // [Pokemon Mystery Dungeon: Explorers of Sky]: ParseDseEvent_Stop resumes at the saved Repeat
        // cursor without resetting octave, previous note duration, or previous wait duration.
        const bool shouldContinue = addLoopForever(beginOffset, 1, "Stop / Repeat");
        curOffset = repeatOffset;
        return shouldContinue;
      }
      addEndOfTrack(beginOffset, curOffset - beginOffset);
      return false;
    case 0x99:
      repeatOffset = curOffset;
      addGenericEvent(beginOffset, curOffset - beginOffset, "Main Loop Begin", "", VGMItem::Type::Marker);
      break;
    case 0x9C: {
      if (curOffset >= offset() + length() || dseLoopStack.size() >= 4) {
        addUnknown(beginOffset, 1, "Loop Top", "Missing count or loop nesting exceeds four frames");
        return false;
      }
      // [Luminous Arc]: ARM9 0x020c30dc pushes one of four 12-byte loop frames. Loop End decrements
      // the byte counter before testing it, so an encoded zero means 256 total passes.
      const uint8_t count = readByte(curOffset++);
      dseLoopStack.push_back({curOffset, 0, static_cast<uint16_t>(count == 0 ? 256 : count), currentOctave});
      addGenericEvent(beginOffset, 2, "Loop Top", fmt::format("Count: {}", dseLoopStack.back().remainingCount),
                      VGMItem::Type::Loop);
    } break;
    case 0x9D: {
      if (dseLoopStack.empty()) {
        addUnknown(beginOffset, 1, "Loop End", "Loop stack is empty");
        return false;
      }
      addGenericEvent(beginOffset, 1, "Loop End", "", VGMItem::Type::Loop);
      auto& frame = dseLoopStack.back();
      if (--frame.remainingCount == 0) {
        dseLoopStack.pop_back();
        break;
      }
      frame.escapeOffset = curOffset;
      curOffset = frame.startOffset;
      // [Pokemon Mystery Dungeon: Explorers of Sky]: ARM9 0x02071a88 restores the octave saved by
      // Loop Top. Line Attack Heroes does the same; Luminous Arc's 0x020c310c preserves the current octave.
      if (static_cast<PSDSESeq*>(parentSeq)->version != 0x0402) {
        currentOctave = frame.startOctave;
      }
    } break;
    case 0x9E: {
      if (dseLoopStack.empty()) {
        addUnknown(beginOffset, 1, "Loop Escape", "Loop stack is empty");
        return false;
      }
      const auto& frame = dseLoopStack.back();
      if (frame.remainingCount == 1 && frame.escapeOffset == 0) {
        addUnknown(beginOffset, 1, "Loop Escape", "No preceding Loop End established the destination");
        return false;
      }
      // [Line Attack Heroes]: SsdSeqLoopEscape pops the frame and uses Loop End's saved destination
      // on the final pass. It leaves octave unchanged, as do Luminous Arc and Explorers of Sky.
      addGenericEvent(beginOffset, 1, "Loop Escape", "Skips to after Loop End on the final pass",
                      VGMItem::Type::LoopBreak);
      if (frame.remainingCount == 1) {
    ...
    case 0xA5: {
      ...
      if (bpm == 0) {
        addUnknown(beginOffset, 2, "Tempo", "Zero tempo operand");
        return false;
      }
      // [Luminous Arc]: ARM9 0x020c31b4 divides 60000000 by A5's operand and adds that duration
      // to the current quarter-note period. A4 replaces the period instead. The delta is not BPM.
      uint32_t period = 60000000U / bpm;
      if (status_byte == 0xA5 && static_cast<PSDSESeq*>(parentSeq)->version == 0x0402) {
        period += static_cast<uint32_t>(std::llround(60000000.0 / parentSeq->tempoBPM));
      }
      // [Line Attack Heroes]: SsdSeqTempoAbsolute and SsdSeqTempoRelative are byte-identical and both replace the
      // integer microsecond period; the 0xA5 operand is not a delta.
      addTempo(beginOffset, curOffset - beginOffset, period, status_byte == 0xA4 ? "Tempo Absolute" : "Tempo Relative");
    } break;
    case 0xA8: {
    ...
    // [Line Attack Heroes]: RVL_Default.MAP names B0 through B6 SsdSeqEnvReset, EnvAL, EnvAR, EnvHT, EnvDRSL, EnvSR,
    // and EnvRR. Values above 0x7f are unset, and each 0xff operand in B4 preserves its prior field. A note copies
    // the instrument envelope only when every override remains unset. SF2 and DLS cannot encode these runtime
    // changes, so the decoder retains them as explicit synthesis automation stubs.
    // [Luminous Arc]: ARM9 0x020c32c8..0x020c32f8 implements B0 through B6 as operand-skipping no-ops.
    // Their widths already match the later handlers, but they do not alter this game's envelopes.
    case 0xB0:
    ...
    case 0xB1: {
      const uint8_t value = readByte(curOffset++);
      addGenericEvent(beginOffset, curOffset - beginOffset, "Set Envelope Attack Level",
                      fmt::format("Override: {}; SF2/DLS export stub", value), VGMItem::Type::Unknown);
    } break;
    case 0xB2: {
      const uint8_t value = readByte(curOffset++);
      addGenericEvent(beginOffset, curOffset - beginOffset, "Set Envelope Attack Rate",
                      fmt::format("Override: {}; SF2/DLS export stub", value), VGMItem::Type::Unknown);
    } break;
    case 0xB3: {
      const uint8_t value = readByte(curOffset++);
    ...
    case 0xB4: {
      ...
      addGenericEvent(beginOffset, curOffset - beginOffset, "Set Envelope Decay Rate And Sustain Level",
    ...
    case 0xB5: {
      const uint8_t value = readByte(curOffset++);
      addGenericEvent(beginOffset, curOffset - beginOffset, "Set Envelope Sustain Rate",
                      fmt::format("Override: {}; SF2/DLS export stub", value), VGMItem::Type::Unknown);
    } break;
    case 0xB6: {
      const uint8_t value = readByte(curOffset++);
      addGenericEvent(beginOffset, curOffset - beginOffset, "Set Envelope Release Rate",
                      fmt::format("Override: {}; SF2/DLS export stub", value), VGMItem::Type::Unknown);
    } break;
    case 0xBC: {
      // [Line Attack Heroes]: RVL_Default.MAP names this handler SsdSeqGate. It scales subsequent note durations by
      // value divided by 127, with 127 as the unscaled default.
      noteDurationMultiplier = readByte(curOffset++);
      addGenericEvent(beginOffset, curOffset - beginOffset, "Gate Time",
                      fmt::format("Note duration scale: {}/127", noteDurationMultiplier),
                      VGMItem::Type::DurationChange);
    } break;
    case 0xBE: {
      // [Line Attack Heroes]: SsdSeqModulation forces active constant-envelope LFOs to this signed level. MIDI has
      // no equivalent for the complete runtime LFO state.
    ...
    case 0xBF: {
      const uint8_t value = readByte(curOffset++);
      // [Line Attack Heroes]: SsdSeqSustaion holds notes for values at least 0x40. Disabling it also releases every
      // note currently held by the channel.
      addSustainEvent(beginOffset, curOffset - beginOffset, value >= 0x40 ? 127 : 0, "Sustain");
    } break;
    case 0xC0:
      tieNextNote = true;
      // [Line Attack Heroes]: RVL_Default.MAP names this no-operand handler SsdSeqSlurSwitch. It applies to one note,
      // reusing the current voice without restarting the sample or envelope.
      addGenericEvent(beginOffset, curOffset - beginOffset, "Slur Next Note",
