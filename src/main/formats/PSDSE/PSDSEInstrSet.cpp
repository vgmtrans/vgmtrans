...
bool PSDSEInstrSet::parseInstrPointers() {
  ...
  const uint64_t chunkEnd = static_cast<uint64_t>(m_header.prgiOffset) + 0x10 + m_header.prgiSize;
  if (chunkEnd > static_cast<uint64_t>(offset()) + length() ||
      static_cast<uint32_t>(m_header.nbprgislots) * 2 > m_header.prgiSize) {
    return false;
  }
  const uint32_t chunkDataStart = m_header.prgiOffset + 0x10;
  const uint32_t prgiChunkEnd = static_cast<uint32_t>(chunkEnd);
  ...
  for (int i = 0; i < m_header.nbprgislots; i++) {
    ...
    if (ptroffset != 0) {
      ...
      if (ptroffset < static_cast<uint32_t>(m_header.nbprgislots) * 2 ||
          instrOffset > prgiChunkEnd || prgiChunkEnd - instrOffset < 0x10) {
...
bool PSDSESampColl::parseSampleInfo() {
  const uint32_t wavTableOffset = m_header.waviOffset + 0x10;
  const uint64_t waveTableEnd = static_cast<uint64_t>(wavTableOffset) + m_header.waviSize;
  const int numSlots = m_header.nbwavislots;
  // [Disaster: Day of Crisis]: EXT03_00_CF_FS.SWD has zero wave slots and an empty WAVI chunk. Inventing 128
  // slots interprets subsequent chunk headers as sample pointers and reads beyond the bank.
  if (waveTableEnd > static_cast<uint64_t>(m_header.offset) + m_header.fileLength ||
      waveTableEnd > rawFile()->size() || static_cast<uint32_t>(numSlots) * 2 > m_header.waviSize) {
    return false;
  }
  ...
  for (int i = 0; i < numSlots; i++) {
    ...
    if (relativeOffset != 0) {
      ...
      const uint32_t recordSize = m_header.version == 0x0402 ? 0x20 : 0x30;
      if (relativeOffset < static_cast<uint32_t>(numSlots) * 2 || sampleInfoOffset > waveTableEnd ||
          recordSize > waveTableEnd - sampleInfoOffset) {
        return false;
      }
...
bool PSDSEInstr::loadInstr() {
  ...
  const uint64_t programEnd = static_cast<uint64_t>(pInstrSet->m_header.prgiOffset) + 0x10 +
                              pInstrSet->m_header.prgiSize;
  if (programEnd > rawFile()->size() || offset() > programEnd || programEnd - offset() < 0x10) {
    return false;
  }
  uint32_t splitsOffset = 0;
  uint32_t lfoTableOffset = 0;
  uint16_t nbsplits = 0;
  const uint8_t nblfos = readByte(offset() + 0x0B);
  ...
  if (pInstrSet->m_header.version == 0x0402) {
    ...
    // [Pokemon Mystery Dungeon: Explorers of Sky]: The split selector uses LDRB at program +2, not LDRH.
    // [Line Attack Heroes]: The corresponding SsdGetSplitData routine uses LBZ at +2 in big-endian SWDB banks.
    // [Shiren the Wanderer]: The retail and beta program layout retains this byte-sized split count.
    // [Disaster: Day of Crisis]: The split selector reads the same byte and advances regions by 0x30.
    // [Arc Rise Fantasia]: The split selector reads the same byte and advances regions by 0x30.
    nbsplits = readByte(offset() + 0x02);
    splitsOffset = offset() + 0x10 + (nblfos * 16) + 16;
    lfoTableOffset = offset() + 0x10;
  }

  const uint64_t splitsEnd = static_cast<uint64_t>(splitsOffset) + nbsplits * 48;
  const uint64_t lfosEnd = static_cast<uint64_t>(lfoTableOffset) + nblfos * 16;
  if (splitsEnd > programEnd || lfosEnd > splitsOffset || lfosEnd > programEnd) {
    return false;
  }
  setLength(static_cast<uint32_t>(splitsEnd - offset()));
...
bool PSDSERgn::loadRgn() {
  ...
  ftune = readByte(tuningOffset);
  ctune = static_cast<int8_t>(readByte(tuningOffset + 1));
  rootKey = readByte(tuningOffset + 2);
  vol = readByte(tuningOffset + 4);
  pan = readByte(tuningOffset + 5);
  addChild(tuningOffset, 1, "Fine Tune");
  addChild(tuningOffset + 1, 1, "Coarse Tune");
  addChild(tuningOffset + 2, 1, "Root Key");
  addChild(tuningOffset + 3, 1, "Key Transpose");
  ...
  // [Line Attack Heroes]: RVL_Default.MAP names these fields AL, AR, DR, SL, HT, SR, and RR.
  addChild(offset() + 0x28, 1, "Attack Level");
  addChild(offset() + 0x29, 1, "Attack Rate");
  addChild(offset() + 0x2a, 1, "Decay Rate");
  ...
  addUnityKey(rootKey, tuningOffset + 2);
  ...
  // residual tune after removing the DSE driver's 32728.5 Hz reference adjustment.
  {
    ...
    if (coarseTune != 0) {
      addCoarseTune(coarseTune, tuningOffset + 1);
    }
    if (fineTune != 0) {
      addFineTune(fineTune, tuningOffset);
    }
  }
  if (vol == 0) {
