...
bool HudsonSnesInstrSet::GetInstrPointers() {
    ...
    SNESSampColl *newSampColl =
        new SNESSampColl(HudsonSnesFormat::name, this->rawfile, spcDirAddr, usedSRCNs);
...
HudsonSnesRgn::HudsonSnesRgn(HudsonSnesInstr *instr, HudsonSnesVersion ver, uint32_t offset,
                             uint32_t spcDirAddr, uint32_t addrTuningEntry)
    : VGMRgn(instr, offset, 4), version(ver) {
    uint8_t srcn = GetByte(dwOffset);
