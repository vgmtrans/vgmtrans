...
HudsonSnesInstrSet::HudsonSnesInstrSet(RawFile* file, HudsonSnesVersion ver, uint32_t offset, uint32_t length, uint32_t spcDirAddr, uint32_t addrSampTuningTable, const std::wstring & name) :
...
bool HudsonSnesInstrSet::GetInstrPointers()
{
	...
	for (uint8_t instrNum = 0; instrNum < unLength / 4; instrNum++)
	{
		...
		HudsonSnesInstr * newInstr = new HudsonSnesInstr(this, version, ofsInstrEntry, instrNum, spcDirAddr, addrSampTuningTable, instrName.str());
...
HudsonSnesInstr::HudsonSnesInstr(VGMInstrSet* instrSet, HudsonSnesVersion ver, uint32_t offset, uint8_t instrNum, uint32_t spcDirAddr, uint32_t addrSampTuningTable, const std::wstring& name) :
...
HudsonSnesRgn::HudsonSnesRgn(HudsonSnesInstr* instr, HudsonSnesVersion ver, uint32_t offset, uint32_t spcDirAddr, uint32_t addrTuningEntry) :
