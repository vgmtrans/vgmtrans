...
PSFFile::PSFFile(RawFile *file) : stripBuf(NULL), stripBufSize(PSF_STRIP_BUF_SIZE) {
    stripBuf = new uint8_t[PSF_STRIP_BUF_SIZE];
...
bool PSFFile::ReadExe(uint8_t *buf, size_t len, size_t stripLen) const {
    ...
    int zRet =
        myuncompress(buf, &destlen, exeCompData->data, (uLong)exeCompData->size, (uLong)stripLen);
    if (zRet != Z_OK) {
...
int PSFFile::myuncompress(Bytef *dest, uLongf *destLen, const Bytef *source, uLong sourceLen,
