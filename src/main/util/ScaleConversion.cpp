...
uint8_t Convert7bitPercentVolValToStdMidiVal(uint8_t percentVal) {
    ...
    // In standard MIDI, the attenuation for volume in db is 40*log10(127/val) ==
    // 20*log10(127^2/val^2). (dls1 spec page 14)
...
// Takes a percentage amplitude value - that is one using a -20*log10(percent) scale for db
// attenuation and converts it to a standard midi value that uses -40*log10(x/127) for db
// attenuation
