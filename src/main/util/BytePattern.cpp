...
BytePattern::BytePattern() : ptn_str(NULL), ptn_mask(NULL), ptn_len(0) {}

BytePattern::BytePattern(const char *pattern, size_t length)
    : ptn_str(NULL), ptn_mask(NULL), ptn_len(length) {
    if (length != 0) {
        if (pattern != NULL) {
...
BytePattern::BytePattern(const BytePattern &obj)
    : ptn_str(NULL), ptn_mask(NULL), ptn_len(obj.ptn_len) {
    if (obj.ptn_str != NULL) {
...
bool BytePattern::search(const void *buf, size_t buf_len, size_t &match_offset,
