...
class MidiTrack {
   public:
    ...
    void InsertNoteByDur(uint8_t channel, int8_t key, int8_t vel, uint32_t duration,
                         uint32_t absTime);
    void PurgePrevNoteOffs();
    void PurgePrevNoteOffs(uint32_t absTime);
    void AddControllerEvent(uint8_t channel, uint8_t controllerNum,
                            uint8_t theDataByte);  // This function should be used for only
                                                   // redirection output of MIDI-like formats
    void InsertControllerEvent(uint8_t channel, uint8_t controllerNum, uint8_t theDataByte,
                               uint32_t absTime);  // This function should be used for only
                                                   // redirection output of MIDI-like formats
    // void AddVolMarker(uint8_t channel, uint8_t vol, int8_t priority = PRIORITY_HIGHER);
    // void InsertVolMarker(uint8_t channel, uint8_t vol, uint32_t absTime, int8_t priority =
    // PRIORITY_HIGHER);
    void AddVol(uint8_t channel, uint8_t vol /*, int8_t priority = PRIORITY_MIDDLE*/);
    void InsertVol(uint8_t channel, uint8_t vol,
                   uint32_t absTime /*, int8_t priority = PRIORITY_MIDDLE*/);
    void AddMasterVol(uint8_t channel, uint8_t mastVol /*, int8_t priority = PRIORITY_HIGHER*/);
    void InsertMasterVol(uint8_t channel, uint8_t mastVol,
                         uint32_t absTime /*, int8_t priority = PRIORITY_HIGHER*/);
...
class MidiEvent {
   public:
    ...
    uint32_t WriteSysexEvent(std::vector<uint8_t> &buf, uint32_t time, uint8_t *data,
                             size_t dataSize);
    ...
    int8_t priority;
    uint8_t channel;
    uint32_t AbsTime;  // absolute time... the number of ticks from the very beginning of the
                       // sequence at which this event occurs
};
...
class NoteEvent : public MidiEvent {
   public:
    NoteEvent(MidiTrack *prntTrk, uint8_t channel, uint32_t absoluteTime, bool bnoteDown,
              uint8_t theKey, uint8_t theVel = 64);
...
class ControllerEvent : public MidiEvent {
   public:
    ControllerEvent(MidiTrack *prntTrk, uint8_t channel, uint32_t absoluteTime,
                    uint8_t controllerNum, uint8_t theDataByte,
...
class VolumeEvent : public ControllerEvent {
   public:
...
class ExpressionEvent : public ControllerEvent {
   public:
...
class SustainEvent : public ControllerEvent {
   public:
...
class PortamentoEvent : public ControllerEvent {
   public:
...
class PortamentoTimeEvent : public ControllerEvent {
   public:
...
class PanEvent : public ControllerEvent {
   public:
...
class ModulationEvent : public ControllerEvent {
   public:
...
class BreathEvent : public ControllerEvent {
   public:
...
class BankSelectEvent : public ControllerEvent {
   public:
...
class BankSelectFineEvent : public ControllerEvent {
   public:
...
{
public:
        VolEvent(MidiTrack* prntTrk, uint8_t channel, uint32_t absoluteTime, uint8_t theVol, int8_t
thePriority = PRIORITY_MIDDLE); virtual VolEvent* MakeCopy();
        //virtual uint32_t WriteEvent(std::vector<uint8_t> & buf, uint32_t time);
...
{
public:
        VolMarkerEvent(MidiTrack* prntTrk, uint8_t channel, uint32_t absoluteTime, uint8_t theVol,
int8_t thePriority = PRIORITY_HIGHER); virtual VolMarkerEvent* MakeCopy(); virtual uint32_t
WriteEvent(std::vector<uint8_t> & buf, uint32_t time);

        uint8_t vol;
};*/

class MastVolEvent : public MidiEvent {
   public:
...
{
public:
        ExpressionEvent(MidiTrack* prntTrk, uint8_t channel, uint32_t absoluteTime, uint8_t
expression, int8_t thePriority = PRIORITY_HIGHER); virtual ExpressionEvent* MakeCopy(); virtual
uint32_t WriteEvent(std::vector<uint8_t> & buf, uint32_t time);

        uint8_t expression;
};*/

class MonoEvent : public ControllerEvent {
   public:
...
class TimeSigEvent : public MidiEvent {
   public:
    TimeSigEvent(MidiTrack *prntTrk, uint32_t absoluteTime, uint8_t numerator, uint8_t denominator,
...
class EndOfTrackEvent : public MidiEvent {
   public:
...
class MarkerEvent : public MidiEvent {
   public:
    MarkerEvent(MidiTrack *prntTrk, uint8_t channel, uint32_t absoluteTime, const std::string &name,
                uint8_t databyte1, uint8_t databyte2, int8_t thePriority = PRIORITY_MIDDLE)
        : MidiEvent(prntTrk, absoluteTime, channel, thePriority),
          name(name),
          databyte1(databyte1),
          databyte2(databyte2) {}
...
class GMResetEvent : public MidiEvent {
   public:
...
class GM2ResetEvent : public MidiEvent {
   public:
...
class GSResetEvent : public MidiEvent {
   public:
...
class XGResetEvent : public MidiEvent {
   public:
