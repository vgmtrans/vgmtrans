...
SeqSlider<TNumber>::SeqSlider(SeqTrack * track, uint32_t time, uint32_t duration, TNumber initialValue, TNumber targetValue) :
	track(track),
	time(time),
	duration(duration),
...
MasterVolSlider::MasterVolSlider(SeqTrack * track, uint32_t time, uint32_t duration, uint8_t initialValue, uint8_t targetValue) :
	SeqSlider(track, time, duration, initialValue, targetValue)
{
}
...
ExpressionSlider::ExpressionSlider(SeqTrack * track, uint32_t time, uint32_t duration, uint8_t initialValue, uint8_t targetValue) :
	SeqSlider(track, time, duration, initialValue, targetValue)
{
}
