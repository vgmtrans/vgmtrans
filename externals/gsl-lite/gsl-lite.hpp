...
#elif gsl_BETWEEN(gsl_COMPILER_GNUC_VERSION, 1, 500) || \
...
//
//     The macros below are for conditional SFINAE, i.e. they apply SFINAE only if the necessary
//     language support is available. Don't use these macros if language support can be assumed in
//     the given context.
//
//     For functions, prefer return-type SFINAE if possible.
//     If return-type SFINAE is not applicable, use `gsl_REQUIRES_A_()` or `typename std::enable_if<
//     VA, int >::type = 0` in the function template argument list.
//
//     Use `gsl_REQUIRES_T_()` or `typename = typename std::enable_if< VA, gsl::detail::enabler
//     >::type` in class template argument lists.
...
//
    ...
    template <class T gsl_REQUIRES_T_(std::is_pointer<T>::value)>
    using owner = T;
#else
                                          // TODO vNext: remove
    template <class T>
...
#define gsl_STRINGIFY_(x) #x
    ...
    struct fail_fast : public std::logic_error {
        gsl_api explicit fail_fast(char const *message) : std::logic_error(message) {}
    };
    ...
    gsl_api gsl_NORETURN inline void fail_fast_terminate() gsl_noexcept { std::terminate(); }
    ...
    class final_action {
       public:
        ...
        gsl_SUPPRESS_MSGSL_WARNING(f .6) gsl_api virtual ~final_action() gsl_noexcept {
            if (invoke_)
                action_();
        }
    ...
    class final_action_return : public final_action<F> {
       public:
        ...
        gsl_is_delete_access : gsl_api final_action_return(final_action_return const &)
                                   gsl_is_delete;
    ...
    namespace detail {
    template <class T, class U>
    struct is_same_signedness : public std::integral_constant<bool, std::is_signed<T>::value ==
    ...
    gsl_api inline gsl_constexpr14 typename Container::value_type &at(Container & cont,
                                                                      size_t pos) {
    ...
    template <class Container>
    gsl_api inline gsl_constexpr14 typename Container::value_type const &at(Container const &cont,
                                                                            size_t pos) {
        Expects(pos < cont.size());
        return cont[pos];
    }
...
// helper class to figure out the pointed-to type of a pointer
...
#if gsl_HAVE(RVALUE_REFERENCE)
        template <class U
        // In Clang 3.x, `is_constructible<not_null<unique_ptr<X>>, unique_ptr<X>>` tries to
        // instantiate the copy constructor of `unique_ptr<>`, triggering an error. Note that Apple
        // Clang's `__clang_major__` etc. are different from regular Clang.
#if gsl_HAVE(TYPE_TRAITS) && gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG) && \
    !gsl_BETWEEN(gsl_COMPILER_CLANG_VERSION, 1, 400) &&                 \
    !gsl_BETWEEN(gsl_COMPILER_APPLECLANG_VERSION, 1, 1001)
                  // We *have* to use SFINAE with an NTTP arg here, otherwise the overload is
                  // ambiguous.
                  ,
    ...
    !gsl_BETWEEN(gsl_COMPILER_APPLECLANG_VERSION, 1, 1001)
        ...
        //     operator U &&() && { ... }
        //
        // However, having two conversion operators with different return types renders the
        // assignment operator of the result type ambiguous:
        //
    ...
    gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG) && gsl_HAVE(EXPLICIT)
        ...
        template <class U, typename std::enable_if<std::is_constructible<U, T const &>::value &&
                                                       !std::is_convertible<T, U>::value &&
                                                       !gsl::detail::is_not_null_oracle<U>::value,
                                                   int>::type = 0>
        gsl_api gsl_constexpr14 explicit operator U() const
...
#if gsl_HAVE(FUNCTION_REF_QUALIFIER)
        template <class U, typename std::enable_if<std::is_constructible<U, T>::value &&
                                                       !std::is_convertible<T, U>::value &&
                                                       !gsl::detail::is_not_null_oracle<U>::value,
                                                   int>::type = 0>
        ...
        template <class U, typename std::enable_if<std::is_constructible<U, T const &>::value &&
                                                       ...
                                                       !gsl::detail::is_not_null_oracle<U>::value,
                                                   int>::type = 0>
        gsl_api gsl_constexpr14 operator U() const
#if gsl_HAVE(FUNCTION_REF_QUALIFIER)
            &
#endif
        {
        ...
        gsl_api gsl_constexpr14 T const &operator->() const {
            Ensures(data_.ptr_ != gsl_nullptr);
            return data_.ptr_;
        }

        gsl_api gsl_constexpr14 element_type &operator*() const {
            Ensures(data_.ptr_ != gsl_nullptr);
            return *data_.ptr_;
        }

#if gsl_HAVE(RVALUE_REFERENCE)
        // Visual C++ 2013 doesn't generate default move constructors, so we declare them
        // explicitly.
        ...
        gsl_is_delete_access : gsl_api not_null() gsl_is_delete;
    ...
    class not_null_ic : public not_null<T> {
       public:
        template <class U gsl_REQUIRES_A_((std::is_constructible<T, U>::value))>
    ...
    template <class T, class U>
    gsl_api inline gsl_constexpr gsl_DECLTYPE_(bool,
    ...
    template <class T, class U>
    gsl_api inline gsl_constexpr gsl_DECLTYPE_(
    ...
    gsl_api std::basic_ostream<CharType, Traits> &operator<<(
...
//
    ...
    class span {
       ...
       public:
...
#else
        template <size_t N gsl_REQUIRES_A_(
        ...
        template <
            class U gsl_REQUIRES_A_((std::is_convertible<U (*)[], element_type (*)[]>::value))>
    ...
    template< class U > operator=();
        ...
        gsl_api gsl_constexpr reference operator[](index_type pos) const { return at(pos); }
    ...
    template <class T, class U>
    gsl_SUPPRESS_MSGSL_WARNING(stl .1) gsl_api inline gsl_constexpr bool operator==(
    ...
    class basic_string_span {
       public:
        ...
        basic_string_span(std::array<typename std11::remove_const<element_type>::type, N> &arr)
            : span_(remove_z(arr)) {}
        ...
        template <class U gsl_REQUIRES_A_(
        ...
        gsl_api gsl_constexpr
        basic_string_span(std::basic_string<typename std11::remove_const<element_type>::type,
    ...
    template <class T, class U>
    gsl_SUPPRESS_MSGSL_WARNING(stl .1) gsl_api inline gsl_constexpr14 bool operator==(
    ...
    template <class T, class U gsl_REQUIRES_A_((!detail::is_basic_string_span<U>::value))>
    gsl_SUPPRESS_MSGSL_WARNING(stl .1) gsl_api inline gsl_constexpr14 bool operator==(
...
gsl_api inline std::basic_string< typename std::remove_const<T>::type > to_string( basic_string_span<T> spn )
{
    ...
    template <class T>
    gsl_api inline span<T> ensure_z(T *const &sz,
