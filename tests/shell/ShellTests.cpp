...
ScanResult scanProbe(const ScanInput& input, bool emptyBank) {
  ...
  sequence.collectionName("Same title").useBank(bank);
...
void persistentSessionAndStableIds() {
  ...
  fixture.ok("assets 0");
  expect(fixture.output.str().find("stream 0 | channels 3") != std::string::npos,
         "sequence inspection should list execution streams and their channels");
  fixture.ok("tree 0 1");
