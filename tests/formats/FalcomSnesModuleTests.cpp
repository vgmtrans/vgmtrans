...
void modulationDynamicAdsrAndEchoRemainPhysical() {
  ...
  expect(performance.diagnostics.empty() && vibratoDepth.size() == 1 && vibratoRate.size() == 1 &&
             ...
             vibratoDepth[0]->context.delay && vibratoDepth[0]->context.delay->ticks == 2 &&
             vibratoDepth[0]->context.shape && vibratoDepth[0]->context.shape->samples.size() == 256 &&
             vibratoRate[0]->context.cyclesPerTick == 0.25 && panDepth.size() == 1 && panDepth[0]->panDepth == 1.0 &&
             panDepth[0]->context.shape && panDepth[0]->context.shape->samples.size() == 32 &&
