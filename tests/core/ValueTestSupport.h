...
[[nodiscard]] bool canScanProbeBadCollectionRole(const SourceFile&, std::span<const u8> bytes) {
  return !bytes.empty() && bytes[0] == 0xf4;
}

[[nodiscard]] ScanResult scanProbeBadCollectionRole(const ScanInput& input) {
  const auto assetId = input.ids.nextAssetId();
  return ScanResult{
      .assets = {InstrumentSetAsset{
          .metadata =
              AssetMetadata{
                  .id = assetId,
                  .format = "ProbeBadCollectionRole",
                  .name = "Instrument set mislabeled as sequence",
                  .range = input.reader.range(0, input.reader.size()),
              },
      }},
      .matchFacts = {MatchFact{
          ...
          .payload =
              CollectionMemberFact{
                  .key = CollectionKey{.resolver = "ProbeBadCollectionRole",
                                       .value = "source:" + std::to_string(input.source.id.value)},
                  .collectionName = input.source.name,
                  .role = CollectionMemberRole::Sequence,
              },
...
[[nodiscard]] FormatModule probeBadCollectionRoleModule() {
  return FormatModule{
      .name = "ProbeBadCollectionRole",
      .canScan = canScanProbeBadCollectionRole,
      .scan = scanProbeBadCollectionRole,
  };
}

[[nodiscard]] bool canScanProbeBadSequenceInstrumentRef(const SourceFile&, std::span<const u8> bytes) {
  return !bytes.empty() && bytes[0] == 0xf5;
}

[[nodiscard]] ScanResult scanProbeBadSequenceInstrumentRef(const ScanInput& input) {
  const auto sequenceId = input.ids.nextAssetId();
  const auto miscId = input.ids.nextAssetId();
  return ScanResult{
      .assets = {SequenceProgramAsset{
                     .metadata =
                         AssetMetadata{
                             .id = sequenceId,
                             .format = "ProbeBadSequenceInstrumentRef",
                             .name = "Bad sequence instrument reference",
                             .range = input.reader.range(0, input.reader.size()),
                         },
                     .program =
                         SequenceProgram{
                             .dialect = DialectId{.value = "probe"},
                             .timebase = Timebase{.ppqn = 48},
                             .referencedInstruments = {SequenceInstrumentRef{
                                 .asset = miscId,
                                 .range = input.reader.range(0, 1),
                             }},
                         },
...
[[nodiscard]] FormatModule probeBadSequenceInstrumentRefModule() {
  return FormatModule{
      .name = "ProbeBadSequenceInstrumentRef",
      .canScan = canScanProbeBadSequenceInstrumentRef,
      .scan = scanProbeBadSequenceInstrumentRef,
  };
}

[[nodiscard]] bool canScanProbeBadRegionSampleRef(const SourceFile&, std::span<const u8> bytes) {
  return !bytes.empty() && bytes[0] == 0xf6;
}

[[nodiscard]] ScanResult scanProbeBadRegionSampleRef(const ScanInput& input) {
  ...
  return ScanResult{
      .assets = {InstrumentSetAsset{
                     .metadata =
                         AssetMetadata{
                             .id = instrumentSetId,
                             .format = "ProbeBadRegionSampleRef",
                             .name = "Bad region sample reference",
                             .range = input.reader.range(0, input.reader.size()),
                         },
                     .instruments = {Instrument{
                         ...
                         .range = input.reader.range(0, input.reader.size()),
                         .regions = {Region{
                             .sample = SampleRef{.collection = miscId, .index = 0},
                             .range = input.reader.range(0, 1),
                         }},
                     }},
                 },
...
[[nodiscard]] FormatModule probeBadRegionSampleRefModule() {
  return FormatModule{
      .name = "ProbeBadRegionSampleRef",
      .canScan = canScanProbeBadRegionSampleRef,
      .scan = scanProbeBadRegionSampleRef,
  };
}
