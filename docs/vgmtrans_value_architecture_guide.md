...
`Allocate Track`, the optional bootstrap rest, and `Open Track` form the SSEQ track-directory prefix rather than a musical track. A small bounded bootstrap reader consumes that prefix once and returns the primary and secondary track starts; ordinary command decoding begins afterward. Sequence-wide bounds, annotations, diagnostics, and recovery policy live in one private decode context, so decoding an individual track requires only its index and start address. The public header exposes only the dialect and whole-sequence parser, not track-walker machinery. Malformed SDAT recovery deliberately remains a specialized walker because malformed files can contain overlapping call-target bytes; it reuses the canonical command decoder and still produces normal `TrackProgram` and source annotations.
