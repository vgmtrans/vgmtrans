...
| KonamiSnes external level factors | Some drivers multiply the voice by global or sound-effect state held outside the sequence. | Note volume, track volume, loop deltas, instrument subtraction, and the versioned lookup table are applied in driver order. | An SPC snapshot does not establish how external game code changes those factors over the whole song. |
| KonamiSnes DSP decrease release | Linear and exponential GAIN releases begin at the voice’s live ENV level. | The raw mode and rate are preserved, with duration represented by the shared full-scale DSP estimate. | Exact duration requires sample-level DSP envelope simulation. |
...
- KonamiSnes `FA`, late `ED`/`FB`, and versioned release commands preserve the distinct physical ADSR2, GAIN, and saved-companion state.
- KonamiSnes level and pan use the driver’s versioned multiply order, divisors, table orientation, and Goemon 3 ROM anomalies.
- Instrument selection restores row envelope, pan, and subtractive volume state; percussion hits reload their own row.
- Terminal low opcodes, V1 defaults, V6 EON controls, legato sentinels, and zero-volume attacks follow their generation-specific dispatch and gating rules.
...
- Proprietary Konami V1 driver: `snes/Konami/Contra III - The Alien Wars.s`
- Proprietary Konami vibrato and effects: `snes/Konami/Axelay.s`
- Annotated Konami V5 driver: `loveemu/sfc-goemon-3-disassembly/disassembly/sound/spc700.asm`
